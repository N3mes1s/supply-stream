...
fn generate_module_files(proto_files: Vec<FileDescriptorProto>) {
    ...
    for proto_file in proto_files {
        if let Some(module_options) = yara_module_options.get(&proto_file.options) {
    ...
    //
    let mut add_modules_rs = File::create("src/modules/add_modules.rs").unwrap();
...
fn generate_proto_code() {
    ...
    if let Ok(proto_files) = env::var("YRX_EXTRA_PROTOS") {
        for path in proto_files.split(' ').collect::<Vec<_>>() {
            let path = if let Ok(base_path) = env::var("YRX_EXTRA_PROTOS_BASE_PATH") {
    ...
    if regenerate {
        generate_module_files(proto_parser.file_descriptor_set().unwrap().file);
...
fn main() {
    if !cfg!(feature = "inventory") && !cfg!(feature = "linkme") {
        panic!("either the `inventory` feature or the `linkme` feature must be enabled.");
    }
