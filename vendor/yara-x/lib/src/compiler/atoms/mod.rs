...
impl Atom {
    ...
    pub fn xor_combinations(self, range: RangeInclusive<u8>) -> XorCombinations {
...
pub(super) fn make_wide(s: &[u8]) -> Vec<u8> {
    itertools::interleave(s.iter().cloned(), itertools::repeat_n(0_u8, s.len())).collect()
}
...
pub(crate) struct CaseCombinations {
    cartesian_product: MultiProduct<smallvec::IntoIter<[u8; DESIRED_ATOM_SIZE]>>,
    backtrack: u16,
    exact: bool,
}
...
impl Iterator for XorCombinations {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut atom = Atom::from(self.atom.bytes.iter().map(|b| b ^ i).collect::<Vec<u8>>());
