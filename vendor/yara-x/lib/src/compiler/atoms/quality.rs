    ...
    I: Iterator<Item = (&'a u8, &'a u8)>,
{
    ...
    fn pop(&mut self) {
        ...
        if quality > self.best_quality {
            ...
            self.best_range = Some(self.queue.front().unwrap().0..self.queue.back().unwrap().0 + 1);
        }
    }
...
impl AtomsQuality {
    ...
    pub fn avg_atom_quality(&self) -> f64 {
        self.sum_atom_quality as f64 / (self.num_inexact_atoms + self.num_exact_atoms) as f64
    }

    pub fn merge(&mut self, other: Self) -> &mut Self {
        self.num_exact_atoms = self.num_exact_atoms.saturating_add(other.num_exact_atoms);

        self.num_inexact_atoms = self
            .num_inexact_atoms
            .saturating_add(other.num_inexact_atoms);

        self.sum_atom_quality = self.sum_atom_quality.saturating_add(other.sum_atom_quality);

        self.min_atom_len = min(self.min_atom_len, other.min_atom_len);

        self.min_atom_quality = min(self.min_atom_quality, other.min_atom_quality);
...
pub(crate) fn best_range_in_bytes(bytes: &[u8]) -> (Option<Range<usize>>, i32) {
...
/// masked slice.
#[allow(dead_code)]
pub(crate) fn best_range_in_masked_bytes(bytes: &[u8], mask: &[u8]) -> (Option<Range<usize>>, i32) {
...
where
    B: IntoIterator<Item = &'a u8>,
{
    BestAtomFinder::new(zip(bytes, iter::repeat(&0xff)))
        .find()
        .1
}
...
mod test {
    ...
    fn best_range_in_masked_bytes() {
        ...
        assert_eq!(
            atoms::best_range_in_masked_bytes(&[0x01, 0x02, 0x00, 0x00], &[0xFF, 0xFF, 0x00, 0x00],),
            (Some(0..2), 44),
        );

        assert_eq!(
            atoms::best_range_in_masked_bytes(&[0x01, 0x02, 0x03, 0x04], &[0xFF, 0xFF, 0x0F, 0xFF],),
            (Some(0..4), 70),
        );

        assert_eq!(
            atoms::best_range_in_masked_bytes(&[0x01, 0x02, 0x00, 0x04], &[0xFF, 0xFF, 0x00, 0xFF]),
            (Some(0..4), 58),
        );
