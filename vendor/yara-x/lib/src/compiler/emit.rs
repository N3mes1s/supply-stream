...
use crate::compiler::ir::{
    Expr, ExprId, ForIn, ForOf, IR, Iterable, MatchAnchor, PatternIdx, Quantifier,
};
use crate::compiler::{
    FieldAccess, ForVars, LiteralId, OfExprTuple, OfPatternSet, PatternId, RegexpId, RuleId,
    RuleInfo, Var,
};
...
use crate::wasm::{
    LOOKUP_INDEXES_END, LOOKUP_INDEXES_START, MATCHING_RULES_BITMAP_BASE, VARS_STACK_START,
    WasmSymbols,
};
...
impl EmitContext<'_> {
    ...
    pub fn function_id(&self, fn_mangled_name: &str) -> FunctionId {
        *self
            .wasm_exports
            .get(fn_mangled_name)
            .unwrap_or_else(|| panic!("can't find function `{fn_mangled_name}`"))
    }
...
fn emit_expr(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    match ir.get(expr) {
        Expr::Const(type_value) => match type_value {
            ...
            TypeValue::String {
                ...
                instr.i64_const(RuntimeString::Literal(literal_id).into_wasm());
            }
            ...
            match symbol.as_ref() {
                ...
                Symbol::Field {
                    ...
                    match type_value {
                        ...
                        TypeValue::Struct(_) | TypeValue::Array(_) | TypeValue::Map(_) => {
                            ctx.lookup_list.push((index, *is_root));
        ...
        Expr::Add { operands, is_float } => {
            emit_arithmetic_op!(ctx, ir, operands, *is_float, I64Add, F64Add, instr);
        }
        Expr::Sub { operands, is_float } => {
            emit_arithmetic_op!(ctx, ir, operands, *is_float, I64Sub, F64Sub, instr);
        }
        Expr::Mul { operands, is_float } => {
            emit_arithmetic_op!(ctx, ir, operands, *is_float, I64Mul, F64Mul, instr);
        }
        Expr::Div { operands, .. } => emit_div(ctx, ir, operands.as_slice(), instr),
        Expr::Mod { operands } => emit_mod(ctx, ir, operands.as_slice(), instr),
        Expr::Shl { lhs, rhs } => {
        ...
        Expr::Contains { lhs, rhs } => {
            ...
            instr.call(ctx.function_id(wasm::export__str_contains.mangled_name));
        }
        Expr::IContains { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_icontains.mangled_name));
        }
        Expr::StartsWith { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_startswith.mangled_name));
        }
        Expr::IStartsWith { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_istartswith.mangled_name));
        }
        Expr::EndsWith { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_endswith.mangled_name));
        }
        Expr::IEndsWith { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_iendswith.mangled_name));
        }
        Expr::IEquals { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_iequals.mangled_name));
        }

        Expr::Matches { lhs, rhs } => {
            emit_operands!(ctx, ir, *lhs, *rhs, instr);
            instr.call(ctx.function_id(wasm::export__str_matches.mangled_name));
        }
        ...
        Expr::OfExprTuple(of_expr_tuple) => emit_of_expr_tuple(ctx, ir, of_expr_tuple, instr),
        ...
        Expr::FuncCall(func_call) => {
            ...
            if func_call.signature().result_may_be_undef() {
                emit_call_and_handle_undef(ctx, instr, ctx.function_id(func_call.mangled_name()));
            } else {
...
fn emit_defined(ctx: &mut EmitContext, ir: &IR, operand: ExprId, instr: &mut InstrSeqBuilder) {
...
fn emit_not(ctx: &mut EmitContext, ir: &IR, operand: ExprId, instr: &mut InstrSeqBuilder) {
    emit_bool_expr(ctx, ir, operand, instr);
    instr.unop(UnaryOp::I32Eqz);
}

/// Emits the code for `and` operations.
fn emit_and(ctx: &mut EmitContext, ir: &IR, operands: &[ExprId], instr: &mut InstrSeqBuilder) {
...
fn emit_or(ctx: &mut EmitContext, ir: &IR, operands: &[ExprId], instr: &mut InstrSeqBuilder) {
...
fn emit_div(ctx: &mut EmitContext, ir: &IR, operands: &[ExprId], instr: &mut InstrSeqBuilder) {
    let mut operands = operands.iter();
    let first_operand = operands.next().unwrap();
...
fn emit_mod(ctx: &mut EmitContext, ir: &IR, operands: &[ExprId], instr: &mut InstrSeqBuilder) {
    let mut operands = operands.iter();
    let first_operand = operands.next().unwrap();
...
/// encountered.
fn emit_lazy_call_to_search_for_patterns(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder) {
        ...
        instr.if_else(
            ...
            |_else| {
                _else
                    ...
                    .call(ctx.function_id(wasm::export__search_for_patterns.mangled_name));
            },
        );
...
fn emit_pattern_match(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    ...
    match anchor {
        ...
        MatchAnchor::At(offset) => {
            ...
            instr.call(ctx.function_id(wasm::export__is_pat_match_at.mangled_name));
        }
        MatchAnchor::In(range) => {
            emit_expr(ctx, ir, range.lower_bound, instr);
            emit_expr(ctx, ir, range.upper_bound, instr);
            instr.call(ctx.function_id(wasm::export__is_pat_match_in.mangled_name));
        }
    }
}

/// Emits the code that returns the number of matches for a pattern.
fn emit_pattern_count(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    emit_lazy_call_to_search_for_patterns(ctx, instr);
    ...
    match range {
        Some(range) => {
            emit_expr(ctx, ir, range.lower_bound, instr);
            emit_expr(ctx, ir, range.upper_bound, instr);
            instr.call(ctx.function_id(wasm::export__pat_matches_in.mangled_name));
        }
        None => {
            instr.call(ctx.function_id(wasm::export__pat_matches.mangled_name));
        }
    }
}

/// Emits the code that returns the offset of matches for a pattern.
fn emit_pattern_offset(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    emit_lazy_call_to_search_for_patterns(ctx, instr);
...
fn emit_pattern_length(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    emit_lazy_call_to_search_for_patterns(ctx, instr);
...
fn emit_check_for_rule_match(ctx: &mut EmitContext, rule_id: RuleId, instr: &mut InstrSeqBuilder) {
...
fn emit_array_indexing(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, array: &Rc<Array>) {
...
fn emit_map_lookup_by_index(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, map: &Rc<Map>) {
    let func = match map.as_ref() {
        Map::IntegerKeys { deputy, .. } => match deputy.as_ref().unwrap().ty() {
            Type::Integer => wasm::export__map_lookup_by_index_integer_integer.mangled_name,
            Type::String => wasm::export__map_lookup_by_index_integer_string.mangled_name,
            Type::Float => wasm::export__map_lookup_by_index_integer_float.mangled_name,
            Type::Bool => wasm::export__map_lookup_by_index_integer_bool.mangled_name,
            Type::Struct => wasm::export__map_lookup_by_index_integer_struct.mangled_name,
            _ => unreachable!(),
        },
        Map::StringKeys { deputy, .. } => match deputy.as_ref().unwrap().ty() {
            Type::Integer => wasm::export__map_lookup_by_index_string_integer.mangled_name,
            Type::String => wasm::export__map_lookup_by_index_string_string.mangled_name,
            Type::Float => wasm::export__map_lookup_by_index_string_float.mangled_name,
            Type::Bool => wasm::export__map_lookup_by_index_string_bool.mangled_name,
            Type::Struct => wasm::export__map_lookup_by_index_string_struct.mangled_name,
            _ => unreachable!(),
        },
    };
...
fn emit_map_lookup(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, map: Rc<Map>) {
    match map.as_ref() {
...
fn emit_of_pattern_set(
    ...
    match (&of.quantifier, &of.items, &of.anchor) {
            ...
            instr.block(I32, |instr| {
                ...
                for (position, range) in ranges.iter().with_position() {
                    ...
                    if end == start {
                        ...
                        instr.i32_const(end);
                        instr.i64_const(1);
                        instr.call(ctx.function_id(wasm::export__pat_range_match.mangled_name));
                    }
        ...
        // single range)
        (Quantifier::Expr(quantifier), _, MatchAnchor::None) if ranges.len() == 1 => {
...
fn emit_for_in_range(ctx: &mut EmitContext, ir: &IR, for_in: &ForIn, instr: &mut InstrSeqBuilder) {
...
fn emit_for_in_expr(ctx: &mut EmitContext, ir: &IR, for_in: &ForIn, instr: &mut InstrSeqBuilder) {
...
fn emit_for_in_array(ctx: &mut EmitContext, ir: &IR, for_in: &ForIn, instr: &mut InstrSeqBuilder) {
    ...
    emit_for(
        ...
        |ctx, instr, n, loop_end| {
            ...
            set_var(ctx, instr, n, |ctx, instr| {
                ...
                instr.call(ctx.function_id(wasm::export__array_len.mangled_name));
            });
...
fn emit_for_in_map(ctx: &mut EmitContext, ir: &IR, for_in: &ForIn, instr: &mut InstrSeqBuilder) {
    ...
    emit_for(
        ...
        |ctx, instr, n, loop_end| {
            ...
            set_var(ctx, instr, n, |ctx, instr| {
                ...
                instr.call(ctx.function_id(wasm::export__map_len.mangled_name));
            });
    ...
    A: FnOnce(&mut EmitContext, &mut InstrSeqBuilder, Var),
{
    ...
    let incr_i_and_repeat = |ctx: &mut EmitContext,
                             instr: &mut InstrSeqBuilder,
                             n: Var,
                             i: Var,
    ...
    F: FnMut(&mut EmitContext, &mut InstrSeqBuilder) -> bool,
{
    ...
    while branch_generator(ctx, &mut branch_expr) {
        branch_blocks.push_back(walrus::ir::Block {
            seq: branch_expr.id(),
        });
    ...
    instr.instr(walrus::ir::Block {
        seq: outermost_block_id,
    });
}
...
fn set_var<B>(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, var: Var, block: B)
where
    B: FnOnce(&mut EmitContext, &mut InstrSeqBuilder),
{
    let (store_kind, alignment) = match var.ty() {
        ...
        Type::Integer | Type::String | Type::Struct | Type::Array | Type::Map | Type::Func => {
...
fn set_vars<B>(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, vars: &[Var], block: B)
where
    B: FnOnce(&mut EmitContext, &mut InstrSeqBuilder),
{
...
fn set_var_undef(ctx: &mut EmitContext, instr: &mut InstrSeqBuilder, var: Var, is_undef: bool) {
...
///   empty (e.g: "").
///
fn emit_bool_expr(ctx: &mut EmitContext, ir: &IR, expr: ExprId, instr: &mut InstrSeqBuilder) {
    emit_expr(ctx, ir, expr, instr);
...
fn catch_undef(
    ...
    instr.block(ty, |block| {
        ...
        ctx.exception_handler_stack
            .push((block.id(), Box::new(catch)));
        expr(ctx, block);
    });
