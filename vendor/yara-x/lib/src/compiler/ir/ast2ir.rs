...
use crate::compiler::errors::{
    ArbitraryRegexpPrefix, AssignmentMismatch, DuplicateModifier, DuplicatePattern,
    EmptyPatternSet, EntrypointUnsupported, InvalidBase64Alphabet, InvalidModifier,
    InvalidModifierCombination, InvalidPattern, InvalidRange, InvalidRegexp, MismatchingTypes,
    MixedGreediness, NumberOutOfRange, SyntaxError, TooManyPatterns, UnexpectedNegativeNumber,
    WrongArguments, WrongType,
};
use crate::compiler::ir::hex2hir::hex_pattern_hir_from_ast;
use crate::compiler::ir::{
    Error, Expr, ExprId, Iterable, LiteralPattern, MatchAnchor, Pattern, PatternFlags,
    PatternInRule, Quantifier, Range, RegexpPattern,
};
use crate::compiler::report::{Level, ReportBuilder};
use crate::compiler::{
    CompileContext, CompileError, FilesizeBounds, ForVars, PatternIdx, TextPatternAsHex, warnings,
};
...
use crate::types::{IntegerConstraint, Map, Regexp, StringConstraint, Type, TypeValue};
...
pub(in crate::compiler) fn text_pattern_from_ast<'src>(
        ...
        Some(modifier @ ast::PatternModifier::Xor { start, end, .. }) => {
            if *end < *start {
                return Err(InvalidRange::build(
                    ctx.report_builder,
                    format!("lower bound ({start}) is greater than upper bound ({end})"),
                    ctx.report_builder.span_to_code_loc(modifier.span()),
                ));
            }
    ...
    let (min_len, note) = if base64.is_some() {
        (
            3,
            Some("`base64` requires that pattern is at least 3 bytes long".to_string()),
        )
    } else if base64wide.is_some() {
        (
            3,
            Some("`base64wide` requires that pattern is at least 3 bytes long".to_string()),
        )
    } else {
        (1, None)
    };
...
pub(in crate::compiler) fn hex_pattern_from_ast<'src>(
    ...
    // breaks.
    if let Some(literal) = hir.as_literal_bytes().and_then(|lit| lit.to_str().ok())
        && literal
            .chars()
            .all(|c| (' '..='~').contains(&c) || c == '\t' || c == '\n' || c == '\r')
    {
        let code_loc = ctx.report_builder.span_to_code_loc(pattern.span());
        let mut warning = TextPatternAsHex::build(ctx.report_builder, code_loc.clone());
...
pub(in crate::compiler) fn regexp_pattern_from_ast<'src>(
    ...
    if pattern.modifiers.ascii().is_some() || pattern.modifiers.wide().is_none() {
        flags |= PatternFlags::Ascii;
    }
    ...
    if pattern.modifiers.nocase().is_some() && pattern.regexp.case_insensitive {
        ...
        ctx.warnings.add(|| {
            warnings::RedundantCaseModifier::build(
                ctx.report_builder,
                ctx.report_builder
                    .span_to_code_loc(pattern.modifiers.nocase().unwrap().span()),
                ctx.report_builder
                    .span_to_code_loc(pattern.regexp.span().subspan(i_pos, i_pos + 1)),
            )
        });
    }
    ...
    let hir = re::parser::Parser::new()
        ...
        .map_err(|err| re_error_to_compile_error(ctx.report_builder, &pattern.regexp, err))?;
...
fn expr_from_ast(ctx: &mut CompileContext, expr: &ast::Expr) -> Result<ExprId, CompileError> {
    let expr = match expr {
        ast::Expr::Entrypoint { span } => {
            let code_loc = ctx.report_builder.span_to_code_loc(span.clone());

            let mut err = EntrypointUnsupported::build(ctx.report_builder, code_loc.clone());

            err.report_mut()
                .new_section(
                    Level::HELP,
                    "use `pe.entry_point`, elf.entry_point` or `macho.entry_point`",
                )
        ...
        ast::Expr::True { .. } => ctx.ir.constant(TypeValue::const_bool_from(true)),

        ast::Expr::False { .. } => ctx.ir.constant(TypeValue::const_bool_from(false)),

        ast::Expr::LiteralInteger(lit) => ctx.ir.constant(TypeValue::const_integer_from(lit.value)),

        ast::Expr::LiteralFloat(lit) => ctx.ir.constant(TypeValue::const_float_from(lit.value)),

        ast::Expr::LiteralString(lit) => ctx
            .ir
            .constant(TypeValue::const_string_from(lit.value.as_bytes())),
        ...
        ast::Expr::Eq(expr) => {
            ...
            let replacement = match (lhs_expr.type_value(), rhs_expr.type_value()) {
                (
                    TypeValue::Bool { .. },
                    TypeValue::Integer {
                        value: Const(0), ..
                    ...
                    ctx.ir.not(lhs),
                    format!("not {}", ctx.report_builder.get_snippet(lhs_span)),
                )),
                (
                    TypeValue::Integer {
                        value: Const(0), ..
            ...
            if let Some((replacement_expr, replacement)) = replacement {
                ...
                let mut warning =
                    warnings::BooleanIntegerComparison::build(ctx.report_builder, code_loc.clone());
        ...
        ast::Expr::Ident(ident) => {
            ...
            match symbol {
                ...
                Symbol::Field {
                    ...
                    let code_loc = ctx.report_builder.span_to_code_loc(ident.span());
                        ...
                        warning
                            ...
                            .new_section(
                                ...
                                notice
                                    .help
                                    .clone()
                                    .unwrap_or("apply the following changes".to_owned()),
                            )
        ...
        ast::Expr::PatternMatch(p) => {
            ...
            match p.identifier.name {
                "$" => {
                    ...
                    ctx.ir
                        .pattern_match_var(ctx.symbol_table.lookup("$").unwrap(), anchor)
                }
                _ => {
                    let at = match anchor {
                        MatchAnchor::At(expr) => {
        ...
        ast::Expr::PatternCount(p) => {
            ...
            match (p.identifier.name, &p.range) {
                ...
                ("#", Some(range)) => {
                    ...
                    ctx.ir
                        .pattern_count_var(ctx.symbol_table.lookup("$").unwrap(), Some(range))
                }
                ("#", None) => ctx
                    .ir
                    .pattern_count_var(ctx.symbol_table.lookup("$").unwrap(), None),
        ...
        ast::Expr::PatternOffset(p) => {
            ...
            match (p.identifier.name, &p.index) {
                // Cases where the identifier is `@`.
                ("@", Some(index)) => {
                    let range = integer_in_range_from_ast(ctx, index, 1..=i64::MAX)?;
                    ctx.ir
                        .pattern_offset_var(ctx.symbol_table.lookup("$").unwrap(), Some(range))
                }
                ("@", None) => ctx
                    .ir
                    .pattern_offset_var(ctx.symbol_table.lookup("$").unwrap(), None),
                // Cases where the identifier is not `@`.
                (_, Some(index)) => {
                    let range = integer_in_range_from_ast(ctx, index, 1..=i64::MAX)?;
                    let (pattern_idx, pattern) = ctx.get_pattern_mut(&p.identifier)?;
        ...
        ast::Expr::PatternLength(p) => {
            ...
            match (p.identifier.name, &p.index) {
                // Cases where the identifier is `!`.
                ("!", Some(index)) => {
                    let index = integer_in_range_from_ast(ctx, index, 1..=i64::MAX)?;
                    ctx.ir
                        .pattern_length_var(ctx.symbol_table.lookup("$").unwrap(), Some(index))
                }
                ("!", None) => ctx
                    .ir
                    .pattern_length_var(ctx.symbol_table.lookup("$").unwrap(), None),
                // Cases where the identifier is not `!`.
                (_, Some(index)) => {
                    let index = integer_in_range_from_ast(ctx, index, 1..=i64::MAX)?;
                    let (pattern_idx, pattern) = ctx.get_pattern_mut(&p.identifier)?;
        ...
        ast::Expr::Lookup(expr) => {
            ...
            match ctx.ir.get(primary).type_value() {
                TypeValue::Array(array) => {
                    let index = non_negative_integer_from_ast(ctx, &expr.index)?;
                ...
                type_value => {
                    return Err(WrongType::build(
                        ...
                        ctx.report_builder.span_to_code_loc(expr.primary.span()),
                        None,
                    ));
                }
            }
...
fn bool_expr_from_ast(ctx: &mut CompileContext, ast: &ast::Expr) -> Result<ExprId, CompileError> {
...
fn of_expr_from_ast(ctx: &mut CompileContext, of: &ast::Of) -> Result<ExprId, CompileError> {
        ...
        && let Some(value) = ctx.ir.get(*expr).try_as_const_integer()
    {
        ...
        if value == 0 {
            ...
            warning
                .report_mut()
                .new_section(Level::HELP, "consider using `none` instead of `0`")
                .patch(
    ...
    if matches!(of.anchor, Some(ast::MatchAnchor::At(_))) {
        let raise_warning = match &quantifier {
            ...
            Quantifier::Percentage(expr) => match ctx.ir.get(*expr).type_value() {
                TypeValue::Integer {
    ...
    let expr = match items {
        ...
        OfItems::BoolExprTuple(exprs) => {
            ctx.ir
                .of_expr_tuple(quantifier, for_vars, next_item_var, exprs, anchor)
        }
        // `x of them`, `x of ($a*, $b)`
        OfItems::PatternSet(pattern_set) => {
            ctx.ir
                .of_pattern_set(quantifier, for_vars, next_item_var, pattern_set, anchor)
        }
    };
...
fn for_of_expr_from_ast(
    ...
    Ok(ctx
        .ir
        .for_of(quantifier, next_pattern_id, for_vars, pattern_set, body))
}
    ...
    let (expected_vars, iterable_ty) = match &iterable {
        ...
        Iterable::Expr(expr) => match ctx.ir.get(*expr).type_value() {
            ...
            TypeValue::Map(map) => match map.as_ref() {
                Map::IntegerKeys { .. } => {
                    (vec![TypeValue::unknown_integer(), map.deputy()], Type::Map)
                }
                Map::StringKeys { .. } => {
                    (vec![TypeValue::unknown_string(), map.deputy()], Type::Map)
                }
            },
            _ => unreachable!(),
        },
...
fn with_expr_from_ast(ctx: &mut CompileContext, with: &ast::With) -> Result<ExprId, CompileError> {
        ...
        if let TypeValue::Func(func) = &type_value {
            if func.is_method() {
                return Err(MethodNotAllowedInWith::build(
                    ctx.report_builder,
                    ctx.report_builder.span_to_code_loc(item.expression.span()),
                ));
            }
            symbols
                .borrow_mut()
        ...
        if !with_vars.used(item.identifier.name) {
            ctx.warnings.add(|| {
                warnings::UnusedIdentifier::build(
                    ctx.report_builder,
                    ctx.report_builder.span_to_code_loc(item.identifier.span()),
                )
            })
        }
    }
...
fn iterable_from_ast(
    ...
    match iter {
        ast::Iterable::Range(range) => Ok(Iterable::Range(range_from_ast(ctx, range)?)),
...
fn anchor_from_ast(
    ...
    match anchor {
        Some(ast::MatchAnchor::At(at_)) => Ok(MatchAnchor::At(non_negative_integer_from_ast(
            ctx, &at_.expr,
        )?)),
        Some(ast::MatchAnchor::In(in_)) => Ok(MatchAnchor::In(range_from_ast(ctx, &in_.range)?)),
        None => Ok(MatchAnchor::None),
    }
}

fn range_from_ast(ctx: &mut CompileContext, range: &ast::Range) -> Result<Range, CompileError> {
    ...
    if let (
        TypeValue::Integer {
            value: Const(lower_bound),
            ..
        },
        TypeValue::Integer {
            value: Const(upper_bound),
            ..
        },
    ) = (
    ...
    ) && lower_bound > upper_bound
    {
        return Err(InvalidRange::build(
            ctx.report_builder,
            format!("lower bound ({lower_bound}) is greater than upper bound ({upper_bound})"),
...
fn pattern_set_from_ast(
    ...
    match pattern_set {
        ...
        ast::PatternSet::Them { span } => {
            ...
            let pattern_indexes: Vec<PatternIdx> = (0..ctx.current_rule_patterns.len())
                .map(|i| i.into())
                .collect();
                    ...
                    .any(|pattern| item.matches(pattern.identifier()))
                {
                    return Err(EmptyPatternSet::build(
                        ...
                        Some(if item.wildcard {
                            ...
                            format!("`{}` doesn't match any pattern identifier", item.identifier,)
                        }),
                    ));
                }
            }
...
fn func_call_from_ast(
    ...
    let func = match symbol {
        ...
        Symbol::Field {
            type_value: TypeValue::Func(func),
            ..
        } => func,
        Symbol::Var {
            type_value: TypeValue::Func(func),
    ...
    let arg_types: Vec<Type> = args.iter().map(|arg| ctx.ir.get(*arg).ty()).collect();
...
fn re_error_to_compile_error(
    ...
    match err {
        ...
        re::parser::Error::ArbitraryPrefix { span } => ArbitraryRegexpPrefix::build(
            report_builder,
            report_builder.span_to_code_loc(
                regexp
                    .span()
        ...
        re::parser::Error::UnsupportedInUnicode { span } => InvalidRegexp::build(
            report_builder,
            err.to_string(),
            report_builder.span_to_code_loc(
                regexp
...
pub(in crate::compiler) fn warn_if_not_bool(ctx: &mut CompileContext, ty: Type, span: Span) {
...
macro_rules! gen_string_op {
    ($name:ident, $variant:ident) => {
        fn $name(ctx: &mut CompileContext, expr: &ast::BinaryExpr) -> Result<ExprId, CompileError> {
            let lhs_span = expr.lhs.span();
            let rhs_span = expr.rhs.span();
                ...
                match constraint {
                        ...
                        if const_string.chars().any(|c| c.is_lowercase()) =>
                    {
                        ...
                        warning.report_mut().patch(
                            ...
                            format!("\"{}\"", const_string.to_string().to_uppercase()),
                        );
                        ...
                        if const_string.chars().any(|c| c.is_uppercase()) =>
                    {
                        ...
                        warning.report_mut().patch(
                            ctx.report_builder
                                .span_to_code_loc(const_string_span.clone()),
                            format!("\"{}\"", const_string.to_string().to_lowercase()),
                        );

                        ctx.warnings.add(|| warning);
                        return;
                    }
                    StringConstraint::ExactLength(n) if const_string.len() != n => {
                        ctx.warnings.add(|| {
                            UnsatisfiableExpression::build(
                                ctx.report_builder,
                                format!("the length of this string is {n}"),
                                format!("the length of this string is {}", const_string.len()),
                                ctx.report_builder
                                    .span_to_code_loc(constrained_string_span.clone()),
                                ctx.report_builder
                                    .span_to_code_loc(const_string_span.clone()),
                                None,
                            )
    ...
    let check_integer_constraints =
        |ctx: &mut CompileContext,
         ...
         constrained_integer_span: Span| {
            for constraint in constraints {
                match constraint {
                    IntegerConstraint::Range(min, max) if !(min..=max).contains(&const_integer) => {
                        ctx.warnings.add(|| {
                                UnsatisfiableExpression::build(
                                    ctx.report_builder,
                                    format!(
