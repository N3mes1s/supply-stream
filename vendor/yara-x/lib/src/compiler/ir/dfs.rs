...
impl<'a> DFSIter<'a> {
    ...
    pub fn new(start: ExprId, ir: &'a IR) -> Self {
        Self {
            ir,
            stack: vec![Event::Enter((start, EventContext::None))],
        }
    }
...
impl<'a> Iterator for DFSIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let next = match next {
            Event::Enter((expr, ctx)) => Event::Enter((expr, self.ir.get(expr), ctx)),
            Event::Leave((expr, ctx)) => Event::Leave((expr, self.ir.get(expr), ctx)),
        };
...
impl<'a> DFSWithScopeIter<'a> {
    ...
    pub fn for_scopes(&self) -> impl Iterator<Item = ExprId> + '_ {
        self.scopes
            .iter()
            .filter_map(|expr_id| match self.ir.get(*expr_id) {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let next = match self.dfs.next()? {
            Event::Enter((expr_id, _, ctx)) => {
                if matches!(ctx, EventContext::Body | EventContext::WithDeclaration) {
...
pub(super) fn dfs_common(expr: &Expr, stack: &mut Vec<Event<(ExprId, EventContext)>>) {
    let push_quantifier = |quantifier: &Quantifier, stack: &mut Vec<_>| match quantifier {
        Quantifier::None => {}
        Quantifier::All => {}
        Quantifier::Any => {}
        Quantifier::Percentage(_) => {}
        Quantifier::Expr(expr) => stack.push(Event::Enter((*expr, EventContext::None))),
    };
    ...
    match expr {
        ...
        Expr::FieldAccess(field_access) => {
            for operand in field_access.operands.iter().rev() {
                stack.push(Event::Enter((*operand, EventContext::FieldAccess)));
            }
        }
        ...
        Expr::ForIn(for_in) => {
            ...
            match &for_in.iterable {
                ...
                Iterable::Expr(expr) => stack.push(Event::Enter((*expr, EventContext::None))),
            }
        ...
        Expr::With(with) => {
            ...
            for (_id, expr) in with.declarations.iter().rev() {
                stack.push(Event::Enter((*expr, EventContext::WithDeclaration)))
            }
        }
    }
}
...
mod tests {
    ...
    fn dfs_with_scope() {
        ...
        let with = ir.with(vec![(var_frame.new_var(Type::Integer), const_1)], with_body);
