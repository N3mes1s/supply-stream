...
pub(in crate::compiler) fn hex_pattern_hir_from_ast(
    ...
    hex_sub_pattern_hir_from_ast(ctx, &pattern.identifier, &pattern.sub_patterns)
}
    ...
    while let Some(token) = ast_tokens.next() {
        match token {
            ...
            ast::HexToken::Alternative(alt) => {
                let mut alternatives = Vec::with_capacity(alt.alternatives.len());

                for alt in &alt.as_ref().alternatives {
                    alternatives.push(hex_sub_pattern_hir_from_ast(ctx, pattern_ident, alt)?);
                }
...
mod tests {
    ...
    use regex_syntax::hir::{Class, ClassBytes, ClassBytesRange, Dot, Hir, HirKind, Repetition};
    use rustc_hash::FxHashSet;

    use yara_x_parser::ast;
    use yara_x_parser::ast::{HexAlternative, HexJump, HexPattern, HexSubPattern, HexToken};
    ...
    fn hex_tokens_to_hir() {
        ...
        pattern.sub_patterns = HexSubPattern(vec![HexToken::Alternative(Box::new(
            HexAlternative::new(vec![
                HexSubPattern(vec![HexToken::Byte(ast::HexByte::new(0x01, 0xff))]),
                HexSubPattern(vec![HexToken::Byte(ast::HexByte::new(0x02, 0xff))]),
            ]),
        ))]);
    ...
    fn class_to_hex() {
        assert_eq!(
            class_to_masked_byte(&hex_byte_to_class(&ast::HexByte::new(0x30, 0xff))),
            Some(hir::HexByte {
                value: 0x30,
                mask: 0xff
            })
        ...
        assert_eq!(
            class_to_masked_byte(&hex_byte_to_class(&ast::HexByte::new(0x05, 0x0f))),
            Some(hir::HexByte {
                value: 0x05,
                mask: 0x0f
            })
        ...
        assert_eq!(
            class_to_masked_byte(&hex_byte_to_class(&ast::HexByte::new(0x03, 0xff))),
            Some(hir::HexByte {
                value: 0x03,
                mask: 0xff
            })
        ...
        if let HirKind::Class(Class::Bytes(class)) = Hir::dot(Dot::AnyByte).kind() {
            assert_eq!(
