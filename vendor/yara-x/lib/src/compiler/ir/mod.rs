...
use crate::compiler::ir::dfs::{DFSIter, DFSWithScopeIter, Event, EventContext, dfs_common};
...
impl Pattern {
    ...
    pub fn anchor_at(&mut self, offset: usize) {
        let is_anchorable = !self.flags().contains(PatternFlags::NonAnchorable);
...
impl IR {
    ...
    pub fn shift_vars(&mut self, expr_id: ExprId, from_index: i32, shift_amount: i32) {
        self.dfs_mut(expr_id, |evt| match evt {
            Event::Enter((_, expr, _)) => expr.shift_vars(from_index, shift_amount),
            Event::Leave(_) => {}
        });
    }
    ...
    pub fn dfs_find<P, C>(&self, start: ExprId, predicate: P, prune_if: C) -> Option<&Expr>
    where
        P: Fn(&Expr) -> bool,
        C: Fn(&Expr) -> bool,
    {
        ...
        F: FnMut(ExprId, u64),
    {
        ...
        let ignore =
            |expr: &Expr| matches!(expr, Expr::Const(_) | Expr::Filesize | Expr::Symbol(_));
    ...
    pub fn find_hoisting_candidates(&self) -> Vec<(ExprId, ExprId)> {
        ...
        while let Some(event) = dfs.next() {
            ...
            let stmt_declaring_var = match dfs.scopes().find(|expr_id| match self.get(*expr_id) {
                Expr::With(with) => with.declarations.iter().any(|(v, _)| v.index() == var),
                Expr::ForIn(for_in) => for_in.variables.iter().any(|v| v.index() == var),
                _ => false,
            }) {
                Some(stmt) => stmt,
                None => continue,
        ...
        while let Some(event) = dfs.next() {
            ...
            match self.get(current_expr_id) {
                ...
                _ if !matches!(ctx, EventContext::FieldAccess) => {
                    let current_depends_on = depends_on[current_expr_id.0 as usize];
    ...
    pub fn hoisting(&mut self) -> ExprId {
        for (expr_id, loop_expr_id) in self.find_hoisting_candidates() {
            ...
            let with_stmt = self.with(vec![(var, var_init_stmt)], loop_expr_id);
...
impl IR {
    ...
    pub fn lookup(&mut self, type_value: TypeValue, primary: ExprId, index: ExprId) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
    ...
    pub fn and(&mut self, mut operands: Vec<ExprId>) -> Result<ExprId, Error> {
        if self.constant_folding {
            ...
            if operands
                .iter()
                .any(|op| self.get(*op).type_value().is_const())
            {
    ...
    pub fn add(&mut self, operands: Vec<ExprId>) -> Result<ExprId, Error> {
        ...
        if self.constant_folding
            && let Some(value) =
                self.fold_arithmetic(operands.as_slice(), is_float, |acc, x| acc + x)?
        {
    ...
    pub fn pattern_match(&mut self, pattern: PatternIdx, anchor: MatchAnchor) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        match &anchor {
            MatchAnchor::None => {}
            MatchAnchor::At(expr) => {
    ...
    pub fn pattern_match_var(&mut self, symbol: Symbol, anchor: MatchAnchor) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        match &anchor {
            MatchAnchor::None => {}
            MatchAnchor::At(expr) => {
        ...
        self.parents.push(ExprId::none());
        self.nodes.push(Expr::PatternMatchVar {
    ...
    pub fn pattern_length(&mut self, pattern: PatternIdx, index: Option<ExprId>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
    ...
    pub fn pattern_length_var(&mut self, symbol: Symbol, index: Option<ExprId>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        if let Some(index) = &index {
            self.parents[index.0 as usize] = expr_id;
        }
        self.parents.push(ExprId::none());
        self.nodes.push(Expr::PatternLengthVar {
            symbol: Box::new(symbol),
            index,
        });
        debug_assert_eq!(self.parents.len(), self.nodes.len());
    ...
    pub fn pattern_offset(&mut self, pattern: PatternIdx, index: Option<ExprId>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        if let Some(index) = &index {
            self.parents[index.0 as usize] = expr_id;
        }
    ...
    pub fn pattern_offset_var(&mut self, symbol: Symbol, index: Option<ExprId>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        if let Some(index) = &index {
            self.parents[index.0 as usize] = expr_id;
        }
        self.parents.push(ExprId::none());
        self.nodes.push(Expr::PatternOffsetVar {
            symbol: Box::new(symbol),
            index,
        });
        debug_assert_eq!(self.parents.len(), self.nodes.len());
    ...
    pub fn pattern_count(&mut self, pattern: PatternIdx, range: Option<Range>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
    ...
    pub fn pattern_count_var(&mut self, symbol: Symbol, range: Option<Range>) -> ExprId {
        let expr_id = ExprId::from(self.nodes.len());
        if let Some(range) = &range {
            self.parents[range.lower_bound.0 as usize] = expr_id;
            self.parents[range.upper_bound.0 as usize] = expr_id;
        ...
        self.parents.push(ExprId::none());
        self.nodes.push(Expr::PatternCountVar {
            symbol: Box::new(symbol),
            range,
        });
        debug_assert_eq!(self.parents.len(), self.nodes.len());
    ...
    pub fn with(&mut self, declarations: Vec<(Var, ExprId)>, body: ExprId) -> ExprId {
...
impl IR {
        ...
        F: FnMut(f64, f64) -> f64,
    {
        ...
        if !operands
            .iter()
            .all(|op| self.get(*op).type_value().is_const())
        {
            return Ok(None);
        }
...
impl Expr {
    ...
    pub fn replace_child(&mut self, child: ExprId, replacement: ExprId) {
        ...
        let replace_in_quantifier = |quantifier: &mut Quantifier| match quantifier {
        ...
        match self {
            ...
            Expr::PatternMatch { anchor, .. } | Expr::PatternMatchVar { anchor, .. } => {
                replace_in_anchor(anchor)
            }

            Expr::PatternCount { range, .. } | Expr::PatternCountVar { range, .. } => {
                if let Some(range) = range {
