...
pub(crate) trait LinterInternal {
    fn check(&self, report_builder: &ReportBuilder, rule: &ast::Rule) -> LinterResult;
}
...
impl Tags {
    ...
    pub(crate) fn from_regex<R: Into<String>>(regex: R) -> Result<Self, regex::Error> {
        let regex = regex.into();
                ...
                if !self.allowed.contains(&tag.name.to_string()) {
                    if self.error {
                        return LinterResult::Err(errors::UnknownTag::build(
                            ...
                            tag.name.to_string(),
                            Some(format!("allowed tags: {}", self.allowed.join(", "))),
                        ));
                    } else {
