...
use crate::compiler::errors::{
    CompileError, ConflictingRuleIdentifier, CustomError, DuplicateRule, DuplicateTag,
...
use crate::errors::{
    CircularIncludes, IncludeError, IncludeNotAllowed, IncludeNotFound, InvalidWarningCode,
};
...
impl<'a> Compiler<'a> {
    ...
    /// ```
    pub fn add_include_dir<P: AsRef<std::path::Path>>(&mut self, dir: P) -> &mut Self {
        self.include_dirs
        ...
        S: Into<SourceCode<'src>>,
    {
        ...
        let ast = match src.as_str() {
            Ok(src) => {
                ...
                let cst = WarningSuppressionHook::from(cst).hook(|warning, span| {
            ...
            Err(err) => {
                ...
                let err = InvalidUTF8::build(
                    &self.report_builder,
                    self.report_builder
                        .span_to_code_loc(Span(span_start as u32..span_end as u32)),
                );
    ...
    pub fn build(self) -> Rules {
        ...
        let compiled_wasm_mod =
            wasm::runtime::Module::from_binary(wasm::get_engine(), wasm_mod.as_slice())
    ...
    pub fn add_linter<L: linters::Linter + 'a>(&mut self, linter: L) -> &mut Self {
    ...
    #[doc(hidden)]
    pub fn enable_feature<F: Into<String>>(&mut self, feature: F) -> &mut Self {
...
impl Compiler<'_> {
    ...
    fn check_for_existing_identifier(&self, ident: &Ident) -> Result<(), CompileError> {
    ...
    fn check_for_duplicate_tags(&self, tags: &[Ident]) -> Result<(), CompileError> {
    ...
    fn read_included_file(
        ...
        let read_file = |path: PathBuf| -> Result<(Vec<u8>, PathBuf), io::Error> {
            let mut path = path.canonicalize()?;
            let content = fs::read(&path)?;

            if let Ok(cwd) = env::current_dir().and_then(|dir| dir.canonicalize())
        ...
        // include stack.
        if let Some(dir) = self.include_stack.last().and_then(|path| path.parent())
...
impl Compiler<'_> {
            ...
            match item {
                ast::Item::Import(import) => {
                    ...
                    if let Some(existing_import) = already_imported.insert(
                        ...
                        let duplicated_import = self.report_builder.span_to_code_loc(import.span());
                ...
                ast::Item::Include(include) => {
                    ...
                    let (included_src, included_path) = match self.read_included_file(include) {
                    ...
                    let source_id = self.report_builder.get_current_source_id().unwrap();

                    let source_code = SourceCode::from(included_src.as_slice()).with_origin(
    ...
    fn c_rule(&mut self, rule: &ast::Rule) -> Result<(), CompileError> {
        ...
        let mut convert_meta_value = |value: &ast::MetaValue| match value {
            ...
            ast::MetaValue::String((s, _)) => MetaValue::String(self.lit_pool.get_or_intern(s)),
            ast::MetaValue::Bytes((s, _)) => MetaValue::Bytes(self.lit_pool.get_or_intern(s)),
        };
                    ...
                    && Self::common_byte_repetition(literal_bytes)
                {
                    self.warnings.add(|| {
                        warnings::SlowPattern::build(
                            &self.report_builder,
                            self.report_builder.span_to_code_loc(pat.span().clone()),
                            None,
                        )
                    });
                }
        ...
        let mut condition = match condition {
                    ...
                    || self.ignored_modules.contains(unknown.identifier()) =>
            {
                ...
                if let Some(module_name) = self.ignored_rules.get(unknown.identifier()) {
                    self.warnings.add(|| {
                    ...
                    self.ignored_rules
                        .insert(rule.identifier.name.to_string(), module_name.clone());
                } else {
                    self.warnings.add(|| {
        ...
        for pattern in &rule_patterns {
            ...
            // file size bounds.
            let pattern_id = match self.patterns.entry(pattern.pattern().clone()) {
            ...
            patterns.push(PatternInfo {
                kind,
                pattern_id,
                ident_id: self.ident_pool.get_or_intern(pattern.identifier().name),
                is_private: pattern.pattern().flags().contains(PatternFlags::Private),
            });
        ...
        self.rules.push(RuleInfo {
            ...
            ident_ref: self.report_builder.span_to_code_loc(rule.identifier.span()),
        });
        ...
        for (pattern_id, pattern) in izip!(pattern_ids.iter(), rule_patterns.into_iter()) {
            if pending_patterns.contains(pattern_id) {
                ...
                match pattern.into_pattern() {
                    ...
                    Pattern::Regexp(pattern) | Pattern::Hex(pattern) => {
                        if let Err(err) = self.c_regexp_pattern(*pattern_id, pattern, pattern_span)
                        {
                            self.restore_snapshot(snapshot);
                            return Err(err);
                        }
        ...
        emit_rule_condition(&mut ctx, &self.ir, rule_id, condition, &mut self.wasm_mod);
    ...
    fn c_import(&mut self, import: &Import) -> Result<(), CompileError> {
        ...
        let mut symbol_table = self.current_namespace.symbols.as_ref().borrow_mut();
        ...
        if !symbol_table.contains(module_name) {
            symbol_table.insert(module_name, self.root_struct.lookup(module_name).unwrap());
        }
    ...
    fn c_literal_pattern(&mut self, pattern_id: PatternId, pattern: LiteralPattern) {
        ...
        for (main_pattern, best_atom, flags) in main_patterns {
            ...
            if pattern.flags.contains(PatternFlags::Xor) {
                ...
                debug_assert!(!pattern.flags.contains(
                    PatternFlags::Base64 | PatternFlags::Base64Wide | PatternFlags::Nocase,
                ));
                ...
                // not accepted.
                debug_assert!(
                    !pattern.flags.contains(
                        PatternFlags::Base64 | PatternFlags::Base64Wide | PatternFlags::Xor,
                    )
                );
                ...
                .intersects(PatternFlags::Base64 | PatternFlags::Base64Wide)
            {
                ...
                debug_assert!(
                    !pattern.flags.contains(
                        PatternFlags::Xor | PatternFlags::Fullword | PatternFlags::Nocase,
                    )
                );

                if pattern.flags.contains(PatternFlags::Base64) {
                    for (padding, base64_pattern) in
                        base64_patterns(main_pattern, pattern.base64_alphabet.as_deref())
                    {
                        let sub_pattern = if let Some(alphabet) = pattern.base64_alphabet.as_deref()
                        {
                            SubPattern::CustomBase64 {
                                pattern: pattern_lit_id,
                                alphabet: self.lit_pool.get_or_intern(alphabet),
                                padding,
                            }
                        } else {
                            SubPattern::Base64 {
                        ...
                        self.add_sub_pattern(
                            ...
                            sub_pattern,
                            iter::once({
                                let mut atom = best_atom_in_bytes(base64_pattern.as_slice());
                ...
                if pattern.flags.contains(PatternFlags::Base64Wide) {
                    for (padding, base64_pattern) in
                        base64_patterns(main_pattern, pattern.base64wide_alphabet.as_deref())
                    {
                        let sub_pattern =
                            if let Some(alphabet) = pattern.base64wide_alphabet.as_deref() {
                        ...
                        self.add_sub_pattern(
                            ...
                            sub_pattern,
                            iter::once({
                                let mut atom = best_atom_in_bytes(wide.as_slice());
                                // Atoms for base64 patterns are always
                                // inexact, they require verification.
                                atom.make_inexact();
                                atom
    ...
    fn c_regexp_pattern(
        ...
        if !tail.is_empty() {
            ...
            return self.c_chain(pattern_id, &head, &tail, pattern.flags, span);
        }
    ...
    fn c_alternation_literal(
        ...
        let mut process_literal = |literal: &hir::Literal, wide: bool| {
            let pattern_lit_id = self.intern_literal(literal.0.as_bytes(), wide);

            let best_atom = best_atom_in_bytes(self.lit_pool.get_bytes(pattern_lit_id).unwrap());
    ...
    fn c_chain(
        ...
        if let hir::HirKind::Literal(literal) = leading.kind() {
            ...
            if ascii {
                prev_sub_pattern_ascii = self.c_literal_chain_head(pattern_id, literal, flags);
            }

            if wide {
                prev_sub_pattern_wide =
                    self.c_literal_chain_head(pattern_id, literal, flags | SubPatternFlags::Wide);
            };
        } else {
            let mut flags = common_flags;

            let (atoms, is_fast_regexp) = self.c_regexp(leading, span.clone())?;
        ...
        for (i, p) in trailing.iter().enumerate() {
            ...
            if let hir::HirKind::Literal(literal) = p.hir.kind() {
                ...
                let (atoms, is_fast_regexp) = self.c_regexp(&p.hir, span.clone())?;
    ...
    fn c_regexp(
        ...
        let (result, is_fast_regexp) =
            match re::fast::Compiler::new().compile(hir, &mut self.re_code) {
    ...
    fn c_literal_chain_head(
        ...
        let pattern_lit_id =
            self.intern_literal(literal.0.as_bytes(), flags.contains(SubPatternFlags::Wide));
        self.add_sub_pattern(
            pattern_id,
            ...
            extract_atoms(self.lit_pool.get_bytes(pattern_lit_id).unwrap(), flags),
            SubPatternAtom::from_atom,
        )
    }
...
impl SubPattern {
    ...
    pub fn chained_to(&self) -> Option<SubPatternId> {
        match self {
            ...
            | SubPattern::RegexpChainTail { chained_to, .. } => Some(*chained_to),
            _ => None,
        }
    }
}
...
impl Warnings {
    ...
    pub fn add(&mut self, f: impl FnOnce() -> Warning) {
        if self.warnings.len() < self.max_warnings {
            ...
            if warn && let Some(spans) = self.suppressed_warnings.get(warning.code()) {
