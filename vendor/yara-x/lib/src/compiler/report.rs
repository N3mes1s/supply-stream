...
use annotate_snippets::{Annotation, AnnotationKind, Group, Snippet, renderer};
...
impl Report {
    ...
    pub(crate) fn labels(&self) -> impl Iterator<Item = Label<'_>> {
        self.labels.iter().map(|(level, code_loc, text)| {
            ...
            // instead of O(N).
            let (line, column) = match byte_offset_to_line_col(code, span.start()) {
                Some((line, column)) => (line, column),
                None => panic!("can't find line and column for span {span} in code:\n{code}",),
            };
    ...
    pub(crate) fn footers(&self) -> impl Iterator<Item = Footer<'_>> {
        self.footers.iter().map(|(level, text)| Footer {
            level: level_as_text(level),
            text,
        })
    }
    ...
    pub(crate) fn new_section<T: Into<String>>(&mut self, level: Level, title: T) -> &mut Self {
...
impl Display for Report {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        let mut group =
            Group::with_title(self.level.clone().primary_title(&self.title).id(self.code));
        ...
        let mut snippet: Snippet<'_, Annotation> =
            Snippet::source(src).path(cache_entry.origin.as_deref().unwrap_or("line"));
        ...
        for (level, label_ref, label) in &self.labels {
            ...
            snippet = snippet.annotation(annotation_kind.span(span_start..span_end).label(label));
        }
...
impl CodeCache {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, HashMap<SourceId, CodeCacheEntry>> {
        self.data.read().unwrap()
    }

    pub fn write(&self) -> RwLockWriteGuard<'_, HashMap<SourceId, CodeCacheEntry>> {
...
impl ReportBuilder {
    ...
    pub fn green_style(&self) -> renderer::Style {
        if self.with_colors {
            renderer::Style::new().fg_color(Some(Color::Ansi(AnsiColor::BrightGreen)))
        } else {
            renderer::Style::new()
        }
    }
...
fn byte_offset_to_line_col(text: &str, byte_offset: usize) -> Option<(usize, usize)> {
