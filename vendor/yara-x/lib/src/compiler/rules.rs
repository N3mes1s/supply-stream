...
use crate::compiler::{
    IdentId, Imports, LiteralId, NamespaceId, PatternId, RegexpId, RuleId, SubPattern, SubPatternId,
};
...
pub struct Rules {
    ...
    pub(in crate::compiler) filesize_bounds: FxHashMap<PatternId, FilesizeBounds>,
...
impl Rules {
        ...
        B: AsRef<[u8]>,
    {
        ...
        let version = u32::from_le_bytes(bytes[version_offset..data_offset].try_into().unwrap());
        ...
        let (mut rules, _len): (Self, usize) =
            bincode::serde::decode_from_slice(&bytes[data_offset..], bincode::config::standard())?;
        ...
        if rules.compiled_wasm_mod.is_none() {
            ...
            rules.compiled_wasm_mod = Some(wasm::runtime::Module::from_binary(
                wasm::get_engine(),
    ...
    pub fn serialize_into<W>(&self, writer: W) -> Result<(), SerializationError>
    where
        W: Write,
    {
    ...
    pub fn deserialize_from<R>(mut reader: R) -> Result<Self, SerializationError>
    where
        R: Read,
    {
        let mut bytes = Vec::new();
    ...
    pub(crate) fn get_regexp(&self, regexp_id: RegexpId) -> Regex {
        ...
        let parser = re::parser::Parser::new().relaxed_re_syntax(self.relaxed_re_syntax);
        ...
        let config = regex_automata::meta::Config::new().nfa_size_limit(Some(50 * 1024 * 1024));

        regex_automata::meta::Builder::new()
            .configure(config)
            .build_from_hir(&hir)
            .unwrap_or_else(|err| panic!("error compiling regex `{}`: {:#?}", re.as_str(), err))
    }

    /// Returns a sub-pattern by [`SubPatternId`].
    #[inline]
    pub(crate) fn get_sub_pattern(&self, sub_pattern_id: SubPatternId) -> &(PatternId, SubPattern) {
    ...
    pub(crate) fn ac_automaton(&self) -> &AhoCorasick {
        self.ac
            .as_ref()
            .expect("Aho-Corasick automaton not compiled")
    }
    ...
    pub(crate) fn build_ac_automaton(&mut self) {
        ...
        self.ac = Some(AhoCorasick::new(atoms).expect("failed to build Aho-Corasick automaton"));
    ...
    pub(crate) fn globals(&self) -> types::Struct {
        let (globals, _): (types::Struct, usize) = bincode::serde::decode_from_slice(
    ...
    #[inline]
    pub(crate) fn filesize_bounds(&self, pattern_id: PatternId) -> Option<&FilesizeBounds> {
    ...
    D: Deserializer<'de>,
{
    ...
    let module = if let Some(bytes) = bytes {
        unsafe { wasm::runtime::Module::deserialize(wasm::get_engine(), bytes).ok() }
    } else {
        None
    };
...
impl fmt::Debug for Rules {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for (id, rule) in self.rules.iter().enumerate() {
            ...
            let namespace = self.ident_pool.get(rule.namespace_ident_id).unwrap();
            writeln!(f, "RuleId({id})")?;
...
impl SubPatternAtom {
    ...
    pub(crate) fn from_regexp_atom(sub_pattern_id: SubPatternId, value: RegexpAtom) -> Self {
        Self {
            sub_pattern_id,
            atom: value.atom,
