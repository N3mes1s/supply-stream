...
fn serialization() {
    ...
    assert!(matches!(
        Rules::deserialize(&data).err().unwrap(),
        SerializationError::InvalidVersion {
            expected: _,
            actual: 0
        }
    ));
...
fn linter_tag_list() {
    assert!(
        Compiler::new()
            ...
            .add_source(r#"rule test : foo bar { strings: $foo = "foo" condition: $foo }"#)
            .unwrap()
            .warnings()
            .is_empty()
    );
    ...
    assert_eq!(
        Compiler::new()
            ...
            .add_source(r#"rule test : axs ers { strings: $foo = "foo" condition: $foo }"#)
            .unwrap()
            .warnings()
            .iter()
            .map(|w| w.to_string())
...
1 | rule test : axs ers { strings: $foo = "foo" condition: $foo }
  ...
  = note: allowed tags: foo, bar"#,
    ...
    assert_eq!(
        Compiler::new()
            .add_linter(
                linters::tags_allowed(vec!["foo".to_string(), "bar".to_string(),]).error(true)
            )
            .add_source(r#"rule test : axs ers { strings: $foo = "foo" condition: $foo }"#)
            .expect_err("expected error")
            .to_string(),
...
fn linter_tags_regexp() {
    assert!(
        Compiler::new()
            ...
            .add_source(r#"rule test : foo1 bar2 { strings: $foo = "foo" condition: $foo }"#)
            .unwrap()
            .warnings()
            .is_empty()
    );
    ...
    assert_eq!(
        Compiler::new()
            .add_linter(linters::tag_regex("^(foo|bar)").unwrap())
            .add_source(r#"rule test : baz blah { strings: $foo = "foo" condition: $foo }"#)
            .unwrap()
            .warnings()
            .iter()
            .map(|w| w.to_string())
...
fn linter_required_metadata() {
    assert!(
        Compiler::new()
            ...
            .add_source(
                r#"rule r_foo { meta: author = "foo" strings: $foo = "foo" condition: $foo }"#
            )
            .unwrap()
            .warnings()
            .is_empty()
...
#[test]
fn utf8_errors() {
    let mut src = "rule test {condition: true}"
        .to_string()
        .as_bytes()
        .to_vec();
...
fn errors_serialization() {
    let err = Compiler::new()
        .add_source(SourceCode::from("rule test {condition: foo}").with_origin("test.yar"))
        .err()
        .unwrap();
...
fn test_circular_includes() {
    ...
    assert!(err.contains(r"src/compiler/tests/testdata/includes/included_circular.yar"));

    #[cfg(target_family = "windows")]
    assert!(err.contains(r#"src\compiler\tests\testdata\includes\included_circular.yar"#));
}
...
fn test_errors() {
        ...
        .flatten()
    {
        ...
        let err = compile(src.as_str())
            .expect_err(format!("file {in_path:?} should have failed").as_str());
