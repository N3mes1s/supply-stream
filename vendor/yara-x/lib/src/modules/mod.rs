...
pub mod mods {
    ...
    */
    ...
    pub fn invoke_with_meta_dyn<T: protobuf::MessageFull>(
        ...
        let (_, module) = super::BUILTIN_MODULES
            .iter()
            .find(|(_, module)| module.root_struct_descriptor.full_name() == proto_name)?;
    ...
    pub mod reflect {
        ...
        impl<'a> Field<'a> {
            fn new(name: &'a str, struct_field: &'a types::StructField) -> Self {
        ...
        impl From<&TypeValue> for Type {
            fn from(type_value: &TypeValue) -> Self {
                match type_value {
                    ...
                    TypeValue::Struct(s) => Type::Struct(Struct::new(s.clone())),
                    TypeValue::Array(a) => Type::Array(Box::new(Type::from(&a.deputy()))),
                    TypeValue::Map(m) => {
                        let key_kind = match **m {
                            Map::IntegerKeys { .. } => Type::Integer,
                            Map::StringKeys { .. } => Type::String,
                        };
                        Type::Map(Box::new(key_kind), Box::new(Type::from(&m.deputy())))
                    }
