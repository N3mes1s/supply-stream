...
pub fn create_binary_from_zipped_ihex<P: AsRef<Path>>(path: P) -> Vec<u8> {
    ...
    let f = File::open(path).unwrap_or_else(|_| panic!("can not open file: {:?}", &path));

    let mut zip =
        zip::ZipArchive::new(f).unwrap_or_else(|_| panic!("can not unzip file: {:?}", &path));
    ...
    let inner_file_name = path_without_zip_ext.file_name().unwrap().to_str().unwrap();
    ...
    inner_file
        .read_to_string(&mut ihex)
        .unwrap_or_else(|_| panic!("can not read ihex content from : {:?}", &path));
...
fn test_modules() {
    ...
    files.into_par_iter().for_each(|path| {
        ...
        let metadata_path = path.with_extension("").with_extension("in.metadata.zip");
        ...
        let rule = format!(r#"import "{module_name}" rule test {{ condition: false }}"#);
        ...
        let options = match meta {
            Some(ref meta) => {
                crate::scanner::ScanOptions::new().set_module_metadata(module_name, meta)
            }
        ...
        let output = scan_results
            ...
            .unwrap_or_else(|| panic!("module `{module_name}` should produce some output"));
...
fn test_module_names() {
    ...
    #[cfg(feature = "crate-module")]
    assert_eq!(names.next(), Some("crate"));
