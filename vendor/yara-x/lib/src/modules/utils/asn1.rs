...
use der_parser::asn1_rs::{Any, FromBer, FromDer, OptTaggedParser, ParseResult};
use der_parser::ber::*;
...
#[inline]
pub fn oid_to_object_identifier(oid: &Oid) -> Result<ObjectIdentifier, const_oid::Error> {
...
pub fn oid_to_str(oid: &Oid) -> Cow<'static, str> {
    ...
    match oid {
        ...
        rfc5912::MD_5_WITH_RSA_ENCRYPTION => Cow::Borrowed("md5WithRSAEncryption"),
        rfc5912::SHA_1_WITH_RSA_ENCRYPTION | oid::SHA1_WITH_RSA_ENCRYPTION_OBSOLETE => {
            Cow::Borrowed("sha1WithRSAEncryption")
        }
        rfc5912::SHA_256_WITH_RSA_ENCRYPTION => Cow::Borrowed("sha256WithRSAEncryption"),
        rfc5912::SHA_384_WITH_RSA_ENCRYPTION => Cow::Borrowed("sha384WithRSAEncryption"),
        rfc5912::SHA_512_WITH_RSA_ENCRYPTION => Cow::Borrowed("sha512WithRSAEncryption"),
...
impl<'a> ContentInfo<'a> {
    ...
    pub fn from_ber(data: &'a [u8]) -> Result<Self, nom::Err<der_parser::error::Error>> {
    ...
    fn parse_inner(data: &'a [u8]) -> BerResult<'a, Self> {
        ...
        let (remainder, content) =
            parse_ber_tagged_explicit_g(0, |content, _| parse_ber_any(content))(remainder)?;
...
impl<'a> SignedData<'a> {
    fn parse_inner(input: &'a [u8]) -> BerResult<'a, Self> {
        ...
        let (remainder, _revocation_info) =
            OptTaggedParser::from(1).parse_ber(remainder, |_, data| parse_ber(data))?;

        let (remainder, signer_infos) = parse_ber_set_of_v(SignerInfo::parse)(remainder)?;
    ...
    pub fn parse_certificates(input: &[u8]) -> (&[u8], Vec<Certificate<'_>>) {
        ...
        let mut cert_parser = consumed(|input| X509CertificateParser::new().parse(input));
...
impl<'a> SignerInfo<'a> {
    pub fn parse(input: &'a [u8]) -> BerResult<'a, Self> {
        parse_ber_sequence_defined_g(|input: &[u8], _| Self::parse_inner(input))(input)
    }

    pub fn parse_inner(input: &'a [u8]) -> BerResult<'a, Self> {
        let (remainder, version) = parse_ber_integer(input)?;

        let (remainder, (issuer, serial_number)) = Self::parse_issuer_and_serial_number(remainder)?;

        let (remainder, digest_algorithm) =
            AlgorithmIdentifier::from_ber(remainder).map_err(|_| BerValueError)?;

        let (remainder, signed_attrs) =
            OptTaggedParser::from(0).parse_ber(remainder, |_, raw_attrs| {
                let (remainder, parsed_attrs) = Self::parse_attributes(raw_attrs)?;
        ...
        let (remainder, unsigned_attrs) = OptTaggedParser::from(1)
            .parse_ber(remainder, |_, raw_attrs| Self::parse_attributes(raw_attrs))?;

        let (raw_signed_attrs, signed_attrs) = signed_attrs.unwrap_or_default();
    ...
    fn parse_issuer_and_serial_number(input: &[u8]) -> BerResult<'_, (X509Name<'_>, BigUint)> {
        parse_ber_sequence_defined_g(|input: &[u8], _| {
            let (remainder, issuer) = X509Name::from_der(input).map_err(|_| BerValueError)?;
...
impl<'a> Attribute<'a> {
    pub fn parse(input: &'a [u8]) -> BerResult<'a, Self> {
        parse_ber_sequence_defined_g(|data: &[u8], _| {
            ...
            let (remainder, attr_values) = parse_ber_set_of_v(parse_ber_any)(remainder)?;
    ...
    pub fn parse(input: &'a [u8]) -> BerResult<'a, Self> {
        parse_ber_sequence_defined_g(|input: &[u8], _| {
            let (remainder, algorithm) =
                AlgorithmIdentifier::from_ber(input).map_err(|_| BerValueError)?;
...
impl SpcSpOpusInfo {
    ...
    fn parse_spc_string(input: &[u8]) -> BerResult<'_, String> {
        alt((
            ...
            map_res(
                parse_ber_tagged_implicit(1, parse_ber_content(Tag::Ia5String)),
...
fn string_from_utf16be(v: &[u8]) -> Option<String> {
    ...
    let x: String = char::decode_utf16(codepoints)
        .collect::<Result<_, _>>()
        .ok()?;
