    ...
    fn try_from(spki: &SubjectPublicKeyInfo<'_>) -> Result<Self, Self::Error> {
            ...
            .map_err(|_| PublicKeyError::InvalidAlgorithm)?
        {
            ...
            rfc5912::ID_DSA => {
                ...
                let (_, y) =
                    parse_ber_integer(key_bytes).map_err(|err| PublicKeyError::Der(err.into()))?;

                let (rem, p) = parse_ber_integer(parameters.data)
                    .map_err(|err| PublicKeyError::Der(err.into()))?;

                let (rem, q) =
                    parse_ber_integer(rem).map_err(|err| PublicKeyError::Der(err.into()))?;
            ...
            rfc5912::ID_EC_PUBLIC_KEY => {
                ...
                match oid {
                    rfc5912::SECP_256_R_1 => Ok(Self::EcdsaP256(
                        ecdsa::VerifyingKey::from_sec1_bytes(spki.subject_public_key.as_ref())?,
                    )),
...
impl PublicKey {
    pub fn from_der(algorithm: ObjectIdentifier, der: &[u8]) -> Result<Self, PublicKeyError> {
        match algorithm {
    ...
    pub fn verify(
        ...
        match oid {
            ...
            | rfc5912::ECDSA_WITH_SHA_256 => self.verify_impl::<Sha256>(message, signature),
            rfc5912::ID_SHA_384
            | rfc5912::SHA_384_WITH_RSA_ENCRYPTION
            | rfc5912::ECDSA_WITH_SHA_384 => self.verify_impl::<Sha384>(message, signature),
            rfc5912::ID_SHA_512
            | rfc5912::SHA_512_WITH_RSA_ENCRYPTION
            | rfc5912::ECDSA_WITH_SHA_512 => self.verify_impl::<Sha512>(message, signature),
            _ => {
                #[cfg(feature = "logging")]
                error!("unknown digest algorithm: {:?}", digest_algorithm.oid());
                false
            }
        }
    }

    fn verify_impl<D: Digest + AssociatedOid>(&self, message: &[u8], signature: &[u8]) -> bool {
    ...
    pub fn verify_digest<D: Digest + AssociatedOid>(
        ...
        match self {
            ...
            Self::Dsa(key) => {
                ...
                dsa::Signature::from_der(signature)
                    .is_ok_and(|s| key.verify_prehash(hash.as_slice(), &s).is_ok())
            }
