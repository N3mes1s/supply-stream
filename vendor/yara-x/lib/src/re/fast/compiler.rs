...
impl Compiler {
    ...
    pub fn compile(
        ...
        for (i, piece) in pieces.iter().enumerate() {
            ...
            match piece {
                ...
                PatternPiece::Pattern(Pattern::Masked(bytes, mask)) => {
                    find_best_atoms(bytes.as_slice(), Some(mask.as_slice()), &mut piece_atoms);
                }
            ...
            let quality = *piece_atoms.iter().map(|(_, _, _, q)| q).min().unwrap();
        ...
        let min_atom_len = atoms.iter().map(|atom| atom.len()).min().unwrap_or(0);
    ...
    fn emit_piece(&mut self, piece: &PatternPiece, instr: &mut InstrSeq) -> Result<(), Error> {
        match piece {
            ...
            PatternPiece::Jump(min, max, accept_newlines) => {
                instr.emit_jump(*min as u16, max.map(|max| max as u16), *accept_newlines);
            }
        }
    ...
    fn visit_pre(&mut self, hir: &Hir) -> Result<(), Self::Err> {
        match hir.kind() {
            ...
            HirKind::Class(class) => {
                ...
                match class {
                    Class::Bytes(class) => {
                        ...
                        // byte, like `3?`.
                        if let Some(masked_byte) = re::hir::class_to_masked_byte(class) {
                            ...
                            re::hir::class_to_masked_bytes_alternation(class)
                        {
                            ...
                            self.pieces.push(PatternPiece::Alternation(
                                masked_bytes
                                    .iter()
                                    .map(|b| Pattern::Masked(vec![b.value], vec![b.mask]))
                                    .collect(),
                            ));
                        } else {
                            return Err(Error::FastIncompatible);
            ...
            HirKind::Repetition(rep) => {
                ...
                let any_byte_except_newline = re::hir::any_byte_except_newline(rep.sub.kind());
                ...
                match (rep.min, rep.max) {
                    ...
                    (min, Some(max)) if min == max && max <= 8 && accept_newlines => {
                        for _ in 0..max {
                            self.bytes.push(0);
                            self.mask.push(0);
                        }
                    ...
                    (min, max) => {
                        ...
                        if Some(min) == max {
                            self.pieces
                                .push(PatternPiece::JumpExact(min, accept_newlines));
                        } else {
                            self.pieces
                                .push(PatternPiece::Jump(min, max, accept_newlines));
                        }
                    }
                }
...
impl InstrSeq {
    ...
    pub fn emit_jump(&mut self, min: u16, max: Option<u16>, accept_newlines: bool) {
        if accept_newlines {
        ...
        self.seq
            .write_all(max.unwrap_or(0).to_le_bytes().as_slice())
            .unwrap();
    }
    ...
    pub fn emit_alternation(&mut self, alternatives: &Vec<Pattern>) -> Result<(), Error> {
        ...
        let alternatives_len = alternatives_len.try_into().map_err(|_| Error::TooLarge)?;
