...
impl<'r> FastVM<'r> {
        ...
        C: CodeLoc,
    {
        ...
        while !self.positions.is_empty() {
            let (instr, instr_size) = InstrParser::decode_instr(&self.code[ip..]);
            ...
            match instr {
                ...
                Instr::Literal(literal) => {
                    for (position, _) in self.positions.iter() {
                        ...
                        let is_match = if backwards {
                            ...
                            self.try_match_literal_fwd(&input[*position..], literal, wide)
                        };
                        if is_match {
                            self.next_positions
                ...
                Instr::JumpExactNoNewline(jump) => {
                    for (position, _) in self.positions.iter() {
                        let jump_range = *position..*position + step * jump as usize;
                ...
                | Instr::JumpNoNewlineUnbounded(..) => {
                    ...
                    let range = match instr {
                        ...
                        Instr::JumpNoNewlineUnbounded(range) => range.start..=self.scan_limit,
                        _ => unreachable!(),
                    };
...
impl FastVM<'_> {
    #[inline]
    fn try_match_literal_fwd(&self, input: &[u8], literal: &[u8], wide: bool) -> bool {
        if wide {
    ...
    #[inline]
    fn try_match_literal_bck(&self, input: &[u8], literal: &[u8], wide: bool) -> bool {
        if wide {
            // The input must be at least twice the length of the literal,
            // because of the interleaved zeroes.
            if input.len() < literal.len() * 2 {
            ...
            for (chunk, byte) in izip!(input.rchunks_exact(2), literal.iter().rev()) {
                if chunk[0] != *byte || chunk[1] != 0 {
                    return false;
                }
            }
    ...
    fn try_match_masked_literal_bck(
        ...
        if wide {
            ...
            for (input, byte, mask) in izip!(input, literal.iter().rev(), mask.iter().rev()) {
                if input & *mask != *byte & *mask {
                    return false;
                }
            }
    ...
    fn jump_fwd(
        ...
        let step = if flags.contains(JumpFlags::Wide) {
            2
        } else {
            1
        };
    ...
    fn jump_bck(
        ...
        let mut on_match_found = |offset| {
            next_positions.insert(position + n + jmp_range.len() - offset - step, ());
        };
