    ...
    pub fn split_at_large_gaps(self) -> (Self, Vec<ChainedPattern>) {
        ...
        for item in cast!(self.into_kind(), HirKind::Concat) {
            if let HirKind::Repetition(rep) = item.kind() {
                let num_repetitions = rep.max.unwrap_or(u32::MAX).saturating_sub(rep.min);
                    ...
                    && any_byte(rep.sub.as_ref().kind())
                {
                    ...
                    if hir.minimum_len().unwrap_or(0) >= Self::MIN_PATTERN_LENGTH_IN_CHAIN {
        ...
        if chain.is_empty() || hir.minimum_len().unwrap_or(0) >= Self::MIN_PATTERN_LENGTH_IN_CHAIN {
            chain.push(ChainedPattern {
                gap: if let Some(gap_max) = gap_max {
                    ChainedPatternGap::Bounded(gap_min..=gap_max)
                } else {
...
impl Hir {
    ...
    pub fn concat(subs: Vec<Hir>) -> Hir {
        regex_syntax::hir::Hir::concat(subs.into_iter().map(|s| s.inner).collect()).into()
    }

    /// Returns an expression that is a repetition of any byte
    /// that repeats at least `min` times and at most `max` time, w
    pub fn any_byte_repetition(min: u32, max: Option<u32>, greedy: bool) -> Hir {
...
pub(crate) fn class_to_masked_bytes_alternation(c: &ClassBytes) -> Option<Vec<HexByte>> {
    if c.ranges().is_empty() {
        return None;
    }
...
mod tests {
    ...
    fn split() {
        ...
        assert_eq!(
            ...
            Hir::concat(vec![
                Hir::literal([0x01]),
                Hir::any_byte_repetition(0, Some(Hir::PATTERN_CHAINING_THRESHOLD), false),
        ...
        assert_eq!(
            ...
            Hir::concat(vec![
                ...
                Hir::literal([0x05]),
                Hir::any_byte_repetition(10, Some(11 + Hir::PATTERN_CHAINING_THRESHOLD), false),
                Hir::literal([0x06, 0x07]),
            ])
            .split_at_large_gaps(),
            // Output
