...
use regex_syntax::ast::{
    AssertionKind, Ast, ErrorKind, Flag, Literal, LiteralKind, RepetitionKind, RepetitionRange,
};
use thiserror::Error;
...
impl Parser {
    ...
    pub fn parse(&self, regexp: &impl Regexp) -> Result<Hir, Error> {
        ...
        let ast = loop {
            ...
            match parser.parse(re_src.as_ref()) {
                ...
                Err(err) => {
                    ...
                    match err.kind() {
                        ErrorKind::EscapeUnrecognized | ErrorKind::ClassEscapeInvalid => {
                            let span = err.span();
                        ...
                        | ErrorKind::RepetitionCountDecimalEmpty => {
                            ...
                            let curly_brace =
                                re_src.as_ref()[0..=span.start.offset].rfind('{').unwrap();
                            let mut s = re_src.into_owned();
        ...
        let hir =
            translator
                .translate(re_src.as_ref(), &ast)
                .map_err(|err| Error::WrongSyntax {
                    msg: err.kind().to_string(),
...
impl re::ast::Visitor for &mut Validator {
    ...
    fn visit_post(&mut self, ast: &Ast) -> Result<(), Self::Err> {
        match ast {
            ...
            | Ast::ClassUnicode(_)
            | Ast::ClassPerl(_)
            | Ast::ClassBracketed(_) => self.match_kind = MatchKind::NonArbitrary,
            Ast::Dot(hir) => {
            ...
            Ast::Repetition(rep) => {
                if let MatchKind::ArbitraryData(_) = self.match_kind {
                    ...
                    if unbounded {
                        self.match_kind = MatchKind::ArbitraryLengthAndData(rep.span);
                    }
                }
            }
            ...
            Ast::Concat(concat) => {
                ...
                let merge_spans = |a: MatchKind, b: MatchKind| match (a, b) {
                    (MatchKind::ArbitraryData(a), MatchKind::ArbitraryData(b)) => {
                        MatchKind::ArbitraryData(re::ast::Span::new(a.start, b.end))
                    }
                    (MatchKind::ArbitraryLengthAndData(a), MatchKind::ArbitraryData(b))
                    | (MatchKind::ArbitraryData(a), MatchKind::ArbitraryLengthAndData(b)) => {
                        MatchKind::ArbitraryLengthAndData(re::ast::Span::new(a.start, b.end))
                    }
                ...
                if self.stack.is_empty() {
                    if let MatchKind::ArbitraryLengthAndData(span) = items
                        ...
                        .take_while(|match_kind| {
                            matches!(
                                match_kind,
                                MatchKind::ArbitraryLengthAndData(_) | MatchKind::ArbitraryData(_)
                            )
                        })
                        .reduce(merge_spans)
                        .unwrap_or_default()
                    ...
                    self.match_kind = items.into_iter().reduce(merge_spans).unwrap();
                }
            }
        }
        Ok(())
