...
use crate::compiler::{
    Atom, AtomsQuality, DESIRED_ATOM_SIZE, MAX_ATOMS_PER_REGEXP, best_atom_in_bytes,
};
...
impl Compiler {
    ...
    pub fn compile(
        ...
        let atoms = atoms
            ...
            .map(|a| re::RegexpAtom {
                atom: a.atom,
                fwd_code: Some(FwdCodeLoc::from(a.code_loc.fwd + fwd_code_start)),
                bck_code: Some(BckCodeLoc::from(a.code_loc.bck + bck_code_start)),
            })
            .collect();
...
impl Compiler {
    ...
    pub(super) fn compile_internal(
        ...
        let (mut backward_code, mut forward_code, mut atoms) = visit(&hir.inner, self)?;
    ...
    fn reverse_backward_code_chunks(&mut self, n: usize) -> Vec<CodeLoc> {
        ...
        for (location, chunk) in zip(locations.iter_mut(), last_n_chunks.iter()).rev() {
        ...
        for atom in best_atoms.iter_mut() {
            if let Some(adjustment) = chunk_locations.get(&atom.code_loc.bck_seq_id) {
    ...
    fn emit_literal(&mut self, literal: &Literal) -> CodeLoc {
        CodeLoc {
            ...
            bck: self
                .backward_code_mut()
                .emit_literal(literal.0.iter().rev()),
        }
    }

    fn emit_clone(&mut self, start: CodeLoc, end: CodeLoc) -> Result<CodeLoc, Error> {
        ...
        let bck = if self.backward_code().seq_id() == start.bck_seq_id {
            ...
            self.backward_code_mut()
                .emit_bytes(cloned_code.as_slice())?
        };
    ...
    fn patch_instr(&mut self, location: &CodeLoc, offset: CodeLocOffset) {
        self.forward_code_mut()
            .patch_instr(location.fwd, offset.fwd);
        self.backward_code_mut()
            .patch_instr(location.bck, offset.bck);
    }
    ...
    fn patch_split_n<I: ExactSizeIterator<Item = CodeLocOffset>>(
        ...
        self.forward_code_mut()
            .patch_split_n(location.fwd, fwd.into_iter());
        self.backward_code_mut()
            .patch_split_n(location.bck, bck.into_iter());
    }
                ...
                if let Some(class) = class.to_byte_class() {
                    ...
                    for range in class.ranges() {
                        for unicode_char in range.start()..=range.end() {
                            ...
                            lits.push(Hir::literal(unicode_char.encode_utf8(&mut buf).as_bytes()));
                        }
                    }
    ...
    fn visit_post_look(&mut self, look: &Look) -> Result<CodeLoc, Error> {
        Ok(match look {
            ...
            Look::StartLF | Look::StartCRLF => self.emit_instr(Instr::LINE_START)?,
    ...
    fn visit_pre_alternation(&mut self, alternatives: &[Hir]) -> Result<(), Error> {
        // e1|e2|....|eN
        //
        // l0: split_n l1,l2,l3
        // l1: ... code for e1 ...
    ...
    fn visit_post_alternation(&mut self, expressions: &[Hir]) -> Result<CodeLoc, Error> {
        // e1|e2|....|eN
        //
        //         split_n l1,l2,l3
        // l1    : ... code for e1 ...
        ...
        let last_n = self
            .best_atoms_stack
            .split_off(self.best_atoms_stack.len() - n);
    ...
    fn visit_pre_repetition(&mut self, rep: &Repetition) -> Result<(), Error> {
        ...
        match (rep.min, rep.max, rep.greedy, nested_rep) {
            ...
            // l4:
            //
            (0, Some(max), greedy, false) if max > Self::REPEAT_INSTR_THRESHOLD => {
                let l1 = self.emit_instr(if greedy {
                    Instr::SPLIT_A
                } else {
                    Instr::SPLIT_B
            ...
            (min, Some(max), _, false)
                if min > Self::REPEAT_INSTR_THRESHOLD || max > Self::REPEAT_INSTR_THRESHOLD =>
            {
                self.start_backward_code_chunk();
                self.repeats.push(true);
                self.bookmarks.push(self.location());
    ...
    fn visit_post_repetition(&mut self, rep: &Repetition) -> Result<CodeLoc, Error> {
        match (rep.min, rep.max, rep.greedy, self.repeats.pop().unwrap()) {
            ...
            // l4:
            //
            (0, Some(max), greedy, true) if max > Self::REPEAT_INSTR_THRESHOLD => {
                debug_assert!(max > 0);
    ...
    fn visit_post(&mut self, hir: &Hir) -> Result<(), Self::Err> {
        ...
        let (atoms, code_loc) = match hir.kind() {
            ...
            HirKind::Literal(literal) => {
                ...
                let adjustment = literal_code_length(&literal[0..best_atom.backtrack() as usize]);
        ...
        let can_be_exact = self.depth == 0 && hir.properties().look_set().is_empty();
...
impl InstrSeq {
    ...
    pub fn emit_split_n(&mut self, n: NumAlt) -> Result<usize, Error> {
        ...
        self.seq
            .write_all(&[OPCODE_PREFIX, Instr::SPLIT_N])
            .unwrap();
        self.seq
            .write_all(self.split_id.to_le_bytes().as_slice())
            .unwrap();
        ...
        self.seq
            .write_all(NumAlt::to_le_bytes(n).as_slice())
            .unwrap();
    ...
    pub fn emit_class(&mut self, c: &ClassBytes) -> usize {
        ...
        if c.ranges().len() < 16 {
            self.seq
                .write_all(&[OPCODE_PREFIX, Instr::CLASS_RANGES, c.ranges().len() as u8])
                .unwrap();
            ...
            self.seq
                .write_all(&[OPCODE_PREFIX, Instr::CLASS_BITMAP])
                .unwrap();
    ...
    pub fn emit_literal<'a, I: IntoIterator<Item = &'a u8>>(&mut self, literal: I) -> usize {
        let location = self.location();
        for b in literal {
    ...
    /// unique ID.
    pub fn emit_clone(&mut self, start: usize, end: usize) -> Result<usize, Error> {
        let location = self.location();
        ...
        self.seq
            .seek(SeekFrom::Current(end as i64 - start as i64))
            .unwrap();
    ...
    fn update_split_ids(&mut self, location: usize) -> Result<(), Error> {
        ...
        for (instr, offset) in InstrParser::new(code) {
            match instr {
                Instr::SplitA(_, _) | Instr::SplitB(_, _) | Instr::SplitN(_) => {
        ...
        for (instr, addr) in InstrParser::new(self.seq.get_ref().as_slice()) {
            match instr {
                ...
                Instr::MaskedByte { byte, mask } => {
                    writeln!(f, "{addr:05x}: MASKED_BYTE {byte:#04x} {mask:#04x}")?;
                }
                ...
                Instr::SplitN(split) => {
                    ...
                    for offset in split.offsets() {
                        write!(f, " {:05x}", (addr as isize).saturating_add(offset.into()))?;
                    }
                    writeln!(f)?;
                }
