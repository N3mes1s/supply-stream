    ...
    pub fn decode_instr(code: &[u8]) -> (Instr<'_>, usize) {
        match code[..] {
            ...
            [OPCODE_PREFIX, Instr::SPLIT_A, ..] => {
                ...
                let offset = Self::decode_offset(&code[2 + size_of::<SplitId>()..]);
            ...
            [OPCODE_PREFIX, Instr::SPLIT_N, ..] => {
                let id = Self::decode_split_id(&code[2..]);
                let n = Self::decode_num_alt(&code[2 + size_of::<SplitId>()..]);

                let offsets = &code[2 + size_of::<SplitId>() + size_of::<NumAlt>()
                    ..2 + size_of::<SplitId>()
                        + size_of::<NumAlt>()
            ...
            [OPCODE_PREFIX, Instr::REPEAT_GREEDY, ..] => {
                ...
                let max = Self::decode_u32(&code[2 + size_of::<Offset>() + size_of::<u32>()..]);
                (
                    Instr::RepeatGreedy { offset, min, max },
                    2 + size_of::<Offset>() + size_of::<u32>() + size_of::<u32>(),
                )
            }
            ...
            [OPCODE_PREFIX, Instr::CLASS_RANGES, ..] => {
                ...
                let ranges = unsafe { code.get_unchecked(3..3 + size_of::<i16>() * n) };
            ...
            [OPCODE_PREFIX, Instr::WORD_BOUNDARY_NEG, ..] => (Instr::WordBoundaryNeg, 2),
...
impl Iterator for SplitOffsets<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let next = Offset::from_le_bytes((&self.0[..size_of::<Offset>()]).try_into().unwrap());
...
impl<'a> ClassBitmap<'a> {
    ...
    /// Returns true if the class contains the given byte.
    pub fn contains(&self, byte: u8) -> bool {
        unsafe { *BitSlice::<_, Lsb0>::from_slice(self.0).get_unchecked(byte as usize) }
    }
}
