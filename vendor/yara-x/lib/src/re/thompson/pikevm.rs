...
impl<'r> PikeVM<'r> {
        ...
        C: CodeLoc,
    {
        match (start.backwards(), wide) {
            ...
            (false, false) => self.try_match_impl(start, right.iter(), left.iter().rev(), f),
            // Going forward, wide.
            (false, true) => {
            ...
            (true, false) => self.try_match_impl(start, left.iter().rev(), right.iter(), f),
            // Going backward, wide.
            (true, true) => {
                let error_fwd = Cell::new(None);
                let error_bck = Cell::new(None);
        ...
        while !self.threads.is_empty() {
            ...
            for (ip, rep_count) in self.threads.iter() {
                let (instr, instr_size) =
                    InstrParser::decode_instr(unsafe { self.code.get_unchecked(*ip..) });
...
pub struct EpsilonClosureState {
    ...
    /// array is set to 1.
    executed_splits: BitArray<[usize; SplitId::MAX.div_ceil(usize::BITS as usize)]>,
...
pub(crate) fn epsilon_closure<C: CodeLoc>(
    ...
    while let Some((ip, mut rep_count)) = state.threads.pop() {
        let (instr, instr_size) = InstrParser::decode_instr(unsafe { code.get_unchecked(ip..) });
        match instr {
            Instr::AnyByte
            | Instr::Byte(_)
            | Instr::MaskedByte { .. }
            ...
            Instr::SplitA(id, offset) => {
                if !state.executed(id) {
                    ...
                    state
                        .threads
                        .push((apply_offset(ip, instr_size.into()), rep_count));
                }
            }
            ...
            Instr::RepeatGreedy { offset, min, max } => {
                rep_count += 1;
                if rep_count >= min {
                    state.threads.push((apply_offset(ip, instr_size.into()), 0));
                }
                if rep_count < max {
                    state.threads.push((apply_offset(ip, offset), rep_count));
                }
            ...
            Instr::LineStart => {
                let is_match = match (start.backwards(), prev_byte, curr_byte) {
                    ...
                    (false, Some(b'\n'), Some(curr_byte)) => *curr_byte != b'\r',
                    // Going forward, previous byte was \r, this is match if
                    // current byte is not \n.
                    (false, Some(b'\r'), Some(curr_byte)) => *curr_byte != b'\n',
                    // Going backward, no current byte, we are at the start
                    // of the input, this is a match.
                    (true, _, None) => true,
                    ...
                    (true, Some(prev_byte), Some(b'\r')) => *prev_byte != b'\n',
                    _ => false,
                };
                if is_match {
                    state
            ...
            Instr::LineEnd => {
                let is_match = match (start.backwards(), prev_byte, curr_byte) {
                    ...
                    (false, Some(prev_byte), Some(b'\r')) => *prev_byte != b'\n',
                    // Going backward, no previous byte, we are at the end of
                    // the data, this is a match.
                    (true, None, _) => true,
                    ...
                    (true, Some(b'\r'), Some(curr_byte)) => *curr_byte != b'\n',
                    _ => false,
                };
                if is_match {
                    state
            ...
            Instr::WordStart => {
                let is_match = match (start.backwards(), prev_byte, curr_byte) {
                    ...
                    (false, None, Some(c)) | (true, Some(c), None) => is_word_char(*c),
                    _ => false,
                };
                if is_match {
                    state
            ...
            Instr::WordEnd => {
                let is_match = match (start.backwards(), prev_byte, curr_byte) {
                    ...
                    (false, Some(p), None) | (true, Some(p), None) => is_word_char(*p),
                    _ => false,
                };
                if is_match {
                    state
