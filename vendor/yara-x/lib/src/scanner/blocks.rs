...
impl<'r> Scanner<'r> {
    ...
    pub fn scan(&mut self, base: usize, data: &[u8]) -> Result<&mut Self, ScanError> {
        ...
        for (_, match_list) in ctx.pattern_matches.matches_per_pattern() {
            ...
            for match_ in match_list.iter().filter(|match_| match_.base == base) {
    ...
    pub fn finish(&mut self) -> Result<ScanResults<'_, 'r>, ScanError> {
        ...
        ctx.scan_state =
            ScanState::Finished(DataSnippets::MultiBlock(mem::take(&mut self.snippets)));
    ...
    pub fn slowest_rules(&self, n: usize) -> Vec<crate::scanner::ProfilingData<'_>> {
...
impl<'r> Scanner<'r> {
    ...
    fn scan_context_mut<'a>(&mut self) -> &'a mut ScanContext<'r, 'a> {
        unsafe {
            transmute::<&mut ScanContext<'static, 'static>, &mut ScanContext<'r, 'a>>(
