...
use crate::compiler::{
    NamespaceId, PatternId, RegexpId, RuleId, Rules, SubPattern, SubPatternAtom, SubPatternFlags,
    SubPatternId,
};
...
use crate::wasm::runtime::{
    AsContext, AsContextMut, Global, GlobalType, Instance, MemoryType, Mutability, Store,
    ...
    pub fn slowest_rules(&self, n: usize) -> Vec<ProfilingData<'_>> {
        ...
        for (rule, condition_exec_time) in iter::zip(
            ...
            if condition_exec_time + pattern_matching_time > 100_000_000 {
                ...
                let rule = self.compiled_rules.ident_pool().get(rule.ident_id).unwrap();

                result.push(ProfilingData {
                    namespace,
                    rule,
                    condition_exec_time: Duration::from_nanos(*condition_exec_time),
                    pattern_matching_time: Duration::from_nanos(pattern_matching_time),
                });
            }
        }
...
impl ScanContext<'_, '_> {
    ...
    #[inline]
    pub(crate) fn wasm_store_mut<'a>(&mut self) -> &'a mut Store<ScanContext<'static, 'static>> {
        unsafe { self.wasm_store.as_mut() }
    }

    /// Returns true of the regexp identified by the given [`RegexpId`]
    /// matches `haystack`.
    pub(crate) fn regexp_matches(&self, regexp_id: RegexpId, haystack: &[u8]) -> bool {
        self.regexp_cache
            .borrow_mut()
            .entry(regexp_id)
    ...
    pub(crate) fn store_struct(&mut self, s: Rc<Struct>) -> RuntimeObjectHandle {
        let obj_ref = RuntimeObjectHandle(Rc::<Struct>::as_ptr(&s) as i64);
        self.runtime_objects
            .insert_full(obj_ref, RuntimeObject::Struct(s));
        obj_ref
    }
    ...
    pub(crate) fn store_array(&mut self, a: Rc<Array>) -> RuntimeObjectHandle {
        ...
        self.runtime_objects
            .insert_full(obj_ref, RuntimeObject::Array(a));
        obj_ref
    }
    ...
    pub(crate) fn store_map(&mut self, m: Rc<Map>) -> RuntimeObjectHandle {
        ...
        self.runtime_objects
            .insert_full(obj_ref, RuntimeObject::Map(m));
        obj_ref
    }

    pub(crate) fn store_string(&mut self, s: Rc<BString>) -> RuntimeObjectHandle {
        let obj_ref = RuntimeObjectHandle(Rc::<BString>::as_ptr(&s) as i64);
        self.runtime_objects
            .insert_full(obj_ref, RuntimeObject::String(s));
        obj_ref
    }
    ...
    pub(crate) fn eval_conditions(&mut self) -> Result<(), ScanError> {
        ...
        let eval_result = self.wasm_main_func.as_ref().unwrap().call(store, ());
        ...
        match eval_result {
            ...
            Err(err) if err.is::<ScanError>() => Err(err.downcast::<ScanError>().unwrap()),
            Err(err) => panic!("unexpected error while executing WASM main function: {err}"),
        }
    }
    ...
    pub(crate) fn reset(&mut self) {
        ...
        if !self.pattern_matches.is_empty() || !self.matching_rules.is_empty() {
            ...
            let bitmap = BitSlice::<_, Lsb0>::from_slice_mut(
                &mut mem[base..base + num_rules.div_ceil(8) + num_patterns.div_ceil(8)],
            );
        ...
        let timeout_secs = self.scan_timeout.map_or(Self::DEFAULT_SCAN_TIMEOUT, |t| {
            cmp::min(t.as_secs_f32().ceil() as u64, Self::DEFAULT_SCAN_TIMEOUT)
        });

        self.deadline = HEARTBEAT_COUNTER.load(Ordering::Relaxed) + timeout_secs;
    ...
    pub(crate) fn update_time_spent_in_rule(&mut self, rule_id: RuleId) {
        ...
        // be successful.
        if let Some(time_spend_in_rule) = self.time_spent_in_rule.get_mut::<usize>(rule_id.into()) {
            time_spend_in_rule.add_assign(
                self.clock
                    .delta_as_nanos(self.rule_execution_start_time, self.clock.raw()),
            );
        }
    }
    ...
    pub(crate) fn track_rule_no_match(&mut self, rule_id: RuleId) {
        ...
        if rule.is_global
            && let Some(rules) = self.matching_rules_per_ns.get_mut(&rule.namespace_id)
        {
    ...
    pub(crate) fn track_rule_match(&mut self, rule_id: RuleId) {
        ...
        let base = MATCHING_RULES_BITMAP_BASE as usize;
        let bits =
            BitSlice::<u8, Lsb0>::from_slice_mut(&mut mem[base..base + num_rules.div_ceil(8)]);
    ...
    pub(crate) fn track_pattern_match(
        ...
        let bits =
            BitSlice::<u8, Lsb0>::from_slice_mut(&mut mem[base..base + num_patterns.div_ceil(8)]);

        bits.set(pattern_id.into(), true);

        if !self
            .pattern_matches
            .add(pattern_id, match_, replace_if_longer)
        {
    ...
    pub(crate) fn ac_search_loop(
        ...
        for ac_match in ac.find_overlapping_iter(data) {
            ...
            let atom = unsafe { atoms.get_unchecked(ac_match.pattern().as_usize()) };
            ...
            let atom_pos = if let Some(atom_pos) = ac_match.start().checked_sub(atom.backtrack()) {
                atom_pos
            } else {
                continue;
            };
            ...
            let (pattern_id, sub_pattern) = &self.compiled_rules.get_sub_pattern(sub_pattern_id);
            ...
            if !block_scanning_mode
                && let Some(bounds) = self.compiled_rules.filesize_bounds(*pattern_id)
            ...
            match sub_pattern {
                ...
                | SubPattern::LiteralChainTail { pattern, flags, .. } => {
                    let pattern = self.compiled_rules.lit_pool().get_bytes(*pattern).unwrap();
                    ...
                    if verify_literal_match(pattern, data, atom_pos, *flags) {
                        self.handle_sub_pattern_match(
                            ...
                            sub_pattern,
                            *pattern_id,
                            Match::new(atom_pos..atom_pos + pattern.len()).rebase(base),
                        );
                    }
                }
                SubPattern::Regexp { flags, .. }
                ...
                | SubPattern::RegexpChainTail { flags, .. } => {
                    verify_regexp_match(&mut vm, data, atom_pos, atom, *flags, |match_range| {
                        self.handle_sub_pattern_match(
                            sub_pattern_id,
                            sub_pattern,
                            *pattern_id,
                ...
                SubPattern::Xor { pattern, flags } => {
                    ...
                    if let Some(key) = verify_xor_match(pattern, data, atom_pos, atom, *flags) {
                        self.handle_sub_pattern_match(
                            sub_pattern_id,
                            sub_pattern,
                            *pattern_id,
                ...
                | SubPattern::Base64Wide { pattern, padding } => {
                    if let Some(match_range) = verify_base64_match(
                        self.compiled_rules.lit_pool().get_bytes(*pattern).unwrap(),
                        data,
                        (*padding).into(),
                        atom_pos,
                        None,
    ...
    pub(crate) fn search_for_patterns(&mut self) -> Result<(), ScanError> {
        ...
        #[cfg(feature = "rules-profiling")]
        {
            self.rule_execution_start_time += scan_end.saturating_sub(scan_start);
        }

        #[cfg(feature = "logging")]
        {
            log::info!("Scan time: {:?}", self.clock.delta(scan_start, scan_end));
        }
                        ...
                        if verify_literal_match(pattern, data, offset, *flags) {
                            self.handle_sub_pattern_match(
                                *sub_pattern_id,
                                sub_pattern,
                                *pattern_id,
                                Match::new(offset..offset + pattern.len()).rebase(base),
                            );
                        }
                    }
                }
    ...
    fn handle_sub_pattern_match(
        ...
        match sub_pattern {
            ...
            SubPattern::LiteralChainHead { .. } | SubPattern::RegexpChainHead { .. } => {
            ...
            | SubPattern::RegexpChainTail {
                ...
                if self.within_valid_distance(*chained_to, match_.range.start, gap) {
                    if flags.contains(SubPatternFlags::LastInChain) {
                        ...
                        self.verify_chain_of_matches(pattern_id, sub_pattern_id, match_);
                    } else {
    ...
    fn within_valid_distance(
        ...
        if let Some(unconfirmed_matches) = self.unconfirmed_matches.get_mut(&chained_to) {
            for m in unconfirmed_matches {
                match gap {
                    ChainedPatternGap::Bounded(gap) => {
                        let min_gap = *gap.start() as usize;
                        let max_gap = *gap.end() as usize;
                        if (m.range.end + min_gap..=m.range.end + max_gap).contains(&match_start) {
                            return true;
                        }
                    }
            ...
            match &self.compiled_rules.get_sub_pattern(id).1 {
                ...
                | SubPattern::RegexpChainHead { flags, .. } => {
                    ...
                    while let Some(id) = next_pattern_id {
                        if let Some(unconfirmed_matches) = self.unconfirmed_matches.get_mut(&id) {
                            unconfirmed_matches
                                .iter_mut()
                                .for_each(|m| m.chain_length = 0);
                        }
                        let (_, sub_pattern) = self.compiled_rules.get_sub_pattern(id);
                ...
                | SubPattern::RegexpChainTail {
                    ...
                    let unconfirmed_matches = match self.unconfirmed_matches.get_mut(chained_to) {
                        Some(m) => m,
                        None => continue,
                    };
                    ...
                    for m in unconfirmed_matches {
                        let in_range = match gap {
                            ...
                            ChainedPatternGap::Unbounded(gap) => {
                                let min_gap = gap.start as usize;
                                (m.range.end + min_gap..).contains(&current_match.range.start)
                            }
                        };
...
fn verify_literal_match(
    ...
    if flags.intersects(SubPatternFlags::FullwordLeft | SubPatternFlags::FullwordRight)
        && !verify_full_word(scanned_data, &(match_start..match_end), flags, None)
    {
        return false;
    }
...
fn verify_full_word(
    ...
    if flags.contains(SubPatternFlags::Wide) {
        if flags.contains(SubPatternFlags::FullwordLeft)
            ...
            && (scanned_data[match_range.start - 2] ^ xor_key).is_ascii_alphanumeric()
        {
            return false;
        }
        if flags.contains(SubPatternFlags::FullwordRight)
            ...
            && (scanned_data[match_range.end] ^ xor_key).is_ascii_alphanumeric()
        {
            return false;
        }
    } else {
        if flags.contains(SubPatternFlags::FullwordLeft)
            && match_range.start >= 1
            && (scanned_data[match_range.start - 1] ^ xor_key).is_ascii_alphanumeric()
        {
            return false;
        }
        if flags.contains(SubPatternFlags::FullwordRight)
    ...
    if let Some(bck_code) = atom.bck_code() {
        if flags.contains(SubPatternFlags::FastRegexp) {
            vm.fast_vm.try_match(
                ...
                flags.contains(SubPatternFlags::Wide),
                |bck_match_len| {
                    let range = match_start - bck_match_len..match_start + fwd_match_len;
...
fn verify_base64_match(
    ...
    let mut decode_range =
        if let (decode_start, false) = match_start.overflowing_sub(decode_start_delta) {
    ...
    let decoded_pattern = decoded
        .as_ref()
        .ok()?
        .get(padding..padding + pattern.len())?;
...
impl RuntimeObject {
    pub fn as_struct(&self) -> Rc<Struct> {
        if let Self::Struct(s) = self {
            ...
            panic!("calling `as_struct` in a RuntimeObject that is not a struct")
        }
    }
    ...
    pub fn as_array(&self) -> Rc<Array> {
        if let Self::Array(a) = self {
            ...
            panic!("calling `as_array` in a RuntimeObject that is not an array")
        }
    }
...
pub fn create_wasm_store_and_ctx<'r>(
    ...
    // dangling.
    wasm_store.data_mut().wasm_store = NonNull::from(wasm_store.as_ref().deref());
    ...
    // for rules.
    let matching_patterns_bitmap_base = MATCHING_RULES_BITMAP_BASE as u32 + num_rules.div_ceil(8);
