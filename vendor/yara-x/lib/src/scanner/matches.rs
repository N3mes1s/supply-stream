...
impl Match {
    ...
    pub fn rebase(mut self, base: usize) -> Self {
        self.range =
            self.range.start.sub(self.base).add(base)..self.range.end.sub(self.base).add(base);
        self.base = base;
        self
    }
...
impl MatchList {
    ...
    pub fn search(&self, offset: usize) -> Result<usize, usize> {
        self.matches
            .binary_search_by(|m| m.range.start.cmp(&offset))
    }
}
...
impl PatternMatches {
    ...
    pub fn add(&mut self, pattern_id: PatternId, m: Match, replace_if_longer: bool) -> bool {
    ...
    pub fn matches_per_pattern(&self) -> impl Iterator<Item = (&PatternId, &MatchList)> {
        self.matches.iter()
    }
}
