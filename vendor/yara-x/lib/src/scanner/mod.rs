...
impl<'a> ScanOptions<'a> {
    ...
    pub fn set_module_metadata(mut self, module_name: &'a str, metadata: &'a [u8]) -> Self {
...
impl<'r> Scanner<'r> {
    ...
    pub fn set_module_output_raw(
        ...
        self.set_module_output(descriptor.unwrap().parse_from(&mut is).map_err(|err| {
            ScanError::ProtoError {
                module: name.to_string(),
                err,
            }
        })?)
...
impl<'r> Scanner<'r> {
    ...
    fn scan_context_mut<'a>(&mut self) -> &mut ScanContext<'r, 'a> {
        unsafe {
            transmute::<&mut ScanContext<'static, 'static>, &mut ScanContext<'r, '_>>(
                self.wasm_store.data_mut(),
            )
        }
    }

    fn load_file<'a>(&self, path: &Path) -> Result<ScannedData<'a>, ScanError> {
        let mut file = fs::File::open(path).map_err(|err| ScanError::OpenError {
        ...
        let data = if self.use_mmap && size > 500_000_000 {
            mapped_file = unsafe {
                MmapOptions::new()
                    ...
                    .map_err(|err| ScanError::MapError {
                        path: path.to_path_buf(),
                        err,
                    })
            }?;
    ...
    fn scan_impl<'a, 'opts>(
        ...
        for module_name in ctx.compiled_rules.imports() {
            ...
            if let Some(output) = ctx.user_provided_module_outputs.remove(root_struct_name) {
                module_output = Some(output);
            } else {
                let meta: Option<&'opts [u8]> = options
                    .as_ref()
                    .and_then(|options| options.module_metadata.get(module_name).copied());

                if let Some(main_fn) = module.main_fn {
                    module_output =
                        Some(main_fn(ctx.scanned_data().unwrap(), meta).map_err(|err| {
    ...
    pub(crate) fn get(&self, range: Range<usize>) -> Option<&[u8]> {
        match self {
            ...
            Self::MultiBlock(btree) => {
                ...
                let (snippet_offset, snippet_data) = btree.range(..=range.start).next_back()?;
...
impl<'a, 'r> ScanResults<'a, 'r> {
    ...
    /// produce any output.
    pub fn module_output(&self, module_name: &str) -> Option<&'a dyn MessageDyn> {
...
impl<'a, 'r> MatchingRules<'a, 'r> {
    fn new(ctx: &'a ScanContext<'r, 'a>) -> Self {
        Self {
            ...
            len_non_private: ctx.matching_rules.len() - ctx.num_matching_private_rules,
...
impl<'a, 'r> NonMatchingRules<'a, 'r> {
    fn new(ctx: &'a ScanContext<'r, 'a>) -> Self {
        ...
        let matching_rules_bitmap =
            BitSlice::<_, Lsb0>::from_slice(&main_memory[base..base + num_rules / 8 + 1]);
