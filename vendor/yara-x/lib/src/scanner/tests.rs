...
fn matches() {
    ...
    assert_eq!(
        matches,
        [
            ("$a", 0..6, b"foobar".as_slice()),
            ("$b", 3..6, b"bar".as_slice())
        ]
    );
...
fn private_rules() {
    ...
    let mut all_matching_rules = scan_results.matching_rules().include_private(true);
...
fn max_matches_per_pattern() {
    ...
    let scan_results = scanner.scan(b"foofoofoo").expect("scan should not fail");
...
fn set_module_output() {
    ...
    scanner
        .set_module_output_raw("pe", pe_data_raw.as_slice())
        .unwrap();
    let scan_results = scanner.scan(b"").expect("scan should not fail");
    assert_eq!(scan_results.matching_rules().len(), 1);
    ...
    scanner
        .set_module_output_raw(mods::PE::descriptor().full_name(), pe_data_raw.as_slice())
        .unwrap();
    let scan_results = scanner.scan(b"").expect("scan should not fail");
    assert_eq!(scan_results.matching_rules().len(), 1);
}
...
fn scan_file() {
    ...
    let mut scanner = Scanner::new(&rules);
    let scan_results = scanner.scan_file("src/tests/testdata/jumps.bin").unwrap();

    assert_eq!(scan_results.matching_rules().len(), 1);

    let scan_results = scanner
        .scan_file_with_options("src/tests/testdata/jumps.bin", ScanOptions::default())
        .unwrap();
