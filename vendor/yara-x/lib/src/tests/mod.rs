...
macro_rules! test_condition {
    ($condition:literal, $data:expr, $expected_result:expr) => {{
        ...
        src.push_str(format!("rule t {{condition: {} }}", $condition).as_str());
...
macro_rules! pattern_true {
    ($pattern:literal,  $data:expr) => {{
        rule_true!(
            format!("rule test {{ strings: $a = {} condition: $a}}", $pattern).as_str(),
            $data
        );
    }};
}
...
macro_rules! pattern_match {
    ($pattern:literal, $data:expr, $expected_result:expr) => {{
        let src = format!("rule test {{ strings: $a = {} condition: $a}}", $pattern);
...
fn test_comparison_operations() {
    ...
    condition_true!(r#"test_proto2.array_bool[0] == test_proto2.array_bool[0]"#);
...
fn uintxx() {
    let data = [
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0xff, 0xff, 0xff, 0xff, 0xff,
...
fn for_in() {
    ...
    #[cfg(feature = "test_proto2-module")]
    condition_true!(r#"not for any i in (0..test_proto2.int64_undef) : (true)"#);

    #[cfg(feature = "test_proto2-module")]
    condition_true!(r#"for any i in (test_proto2.int64_undef, 0, 1) : (i == 1)"#);
}
...
fn with() {
    ...
    #[cfg(feature = "test_proto2-module")]
    condition_false!(r#"with foo = test_proto2.array_int64[test_proto2.int64_zero]: (foo == 10)"#);
    ...
    #[cfg(feature = "test_proto2-module")]
    condition_true!(r#"with foo = test_proto2.nested: (foo.nested_int64_one == 1 )"#);

    #[cfg(feature = "test_proto2-module")]
    condition_false!(r#"with foo = test_proto2.nested: (foo.nested_int64_one == 0 )"#);

    #[cfg(feature = "test_proto2-module")]
    condition_true!(r#"with foo = test_proto2.uppercase("foo"): (foo == "FOO" )"#);

    #[cfg(feature = "test_proto2-module")]
    condition_false!(r#"with foo = test_proto2.uppercase("foo"): (foo == "FoO" )"#);

    #[cfg(feature = "test_proto2-module")]
    condition_false!(r#"with foo = test_proto2.undef_i64(): (foo == 1)"#);

    #[cfg(feature = "test_proto2-module")]
    condition_true!(r#"not defined with foo = test_proto2.undef_i64(): (foo == 1)"#);
...
fn regexp_wide() {
    ...
    pattern_true!(r#"/fo.{1,3}1234/is wide"#, b"f\0o\0o\x001\x002\x003\x004\0");
    pattern_false!(r#"/fo.{1,3}1234/is wide"#, b"f\0x01o\0o\x01\x02\x03\x04\0");
    ...
    pattern_false!(r"/foo.bar/ wide", b"f\x00o\x00o\x00\x00\xFFb\x00a\x00r\x00");
    ...
    pattern_false!(r"/bar.foo/ wide", b"b\x00a\x00r\x00\x00\xFFf\x00o\x00o\x00");
...
fn test_defined_2() {
    ...
    condition_true!(r#"defined (for any x in (0..10) : (test_proto2.undef_i64() == 0))"#);
}
