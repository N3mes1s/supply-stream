...
impl MangledFnName {
    ...
    pub fn unmangle(&self) -> (Vec<(&str, TypeValue)>, TypeValue) {
        let (_fn_name, arg_names_and_types, ret_type) = self
            .0
            .split('@')
            .collect_tuple()
            .unwrap_or_else(|| panic!("invalid mangled name: `{}`", self.0));

        let mut args = Vec::new();

        if !arg_names_and_types.is_empty() {
            for arg_str in arg_names_and_types.split(',') {
                let (arg_name, arg_type) = if let Some((n, t)) = arg_str.split_once(':') {
                    (n, t)
                } else {
                    panic!("argument name missing in mangled name: `{}`", self.0)
                };

                let mut chars = arg_type.chars().peekable();
                let type_value = self.next_type(&mut chars).unwrap_or_else(|| {
                    panic!("invalid argument type in mangled name: `{}`", self.0)
                });
        ...
        let ret = self
            .next_type(&mut chars)
            .unwrap_or_else(|| panic!("expecting return type in mangled name: `{}`", self.0));
                ...
                while let Some(':') = chars.peek() {
                    ...
                    match chars.next() {
                        Some('R') => {
                            ...
                            constraints.push(IntegerConstraint::Range(min, max));
                        }
                        None | Some(_) => {
                ...
                while let Some(':') = chars.peek() {
                    ...
                    match chars.next() {
                        ...
                        Some('N') => {
                            ...
                            constraints.push(StringConstraint::ExactLength(n as usize));
                        }
                        None | Some(_) => {
                            panic!("invalid mangled name: `{}`", self.0)
                        }
...
impl<T: Into<String>> From<T> for Func {
    ...
    fn from(value: T) -> Self {
        ...
        Self {
            signatures: vec![Rc::new(signature)],
            method_of,
        }
    }
}
...
#[cfg(test)]
mod test {
    use crate::types::{IntegerConstraint, MangledFnName, StringConstraint, TypeValue};
    ...
    fn mangled_name() {
        ...
        assert_eq!(
            ...
            (
                ...
                TypeValue::unknown_string_with_constraints(vec![StringConstraint::Lowercase])
            )
        );
        ...
        assert_eq!(
            ...
            (
                ...
                TypeValue::unknown_string_with_constraints(vec![StringConstraint::Uppercase])
            )
        );
        ...
        assert_eq!(
            ...
            (
                ...
                TypeValue::unknown_string_with_constraints(
                    vec![StringConstraint::ExactLength(16),]
                )
            )
        );
        ...
        assert_eq!(
            ...
            (
                vec![],
                TypeValue::unknown_integer_with_constraints(vec![IntegerConstraint::Range(0, 10),])
            )
        );
        ...
        assert_eq!(
            ...
            (
                vec![],
                TypeValue::unknown_integer_with_constraints(vec![IntegerConstraint::Range(
                    -100, 1000
                ),])
            )
        );
