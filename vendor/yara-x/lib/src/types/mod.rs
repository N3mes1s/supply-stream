...
impl TypeValue {
    ...
    pub fn eq_type(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Array(a), Self::Array(b)) => a.deputy().eq_type(&b.deputy()),
            (Self::Map(a), Self::Map(b)) => match (a.as_ref(), b.as_ref()) {
                (Map::StringKeys { .. }, Map::StringKeys { .. }) => a.deputy().eq_type(&b.deputy()),
    ...
    pub fn cast_to_bool(&self) -> Self {
        match self {
            ...
            Self::Float {
                ...
                value: Value::Var(*f != 0.0),
            },
            Self::Float {
                value: Value::Const(f),
            } => Self::Bool {
                value: Value::Const(*f != 0.0),
            },
            ...
            Self::String {
                ...
                value: Value::Var(!s.is_empty()),
            },
            Self::String {
                value: Value::Const(s),
                ..
            } => Self::Bool {
                value: Value::Const(!s.is_empty()),
            },
    ...
    pub fn as_array(&self) -> Rc<Array> {
        if let TypeValue::Array(array) = self {
            ...
            panic!("called `as_array` on a TypeValue that is not TypeValue::Array, it is: {self:?}")
        }
    }
    ...
    pub fn as_map(&self) -> Rc<Map> {
        if let TypeValue::Map(map) = self {
            ...
            panic!("called `as_map` on a TypeValue that is not TypeValue::Map, it is: {self:?}")
        }
    }
    ...
    #[inline]
    pub fn unknown_string_with_constraints<C: Into<Vec<StringConstraint>>>(constraints: C) -> Self {
        Self::String {
            value: Value::Unknown,
    ...
    /// the given constraints.
    #[inline]
    pub fn unknown_integer_with_constraints<C: Into<Vec<IntegerConstraint>>>(
        constraints: C,
    ) -> Self {
        Self::Integer {
            value: Value::Unknown,
...
impl PartialEq for TypeValue {
    fn eq(&self, rhs: &Self) -> bool {
        match (self, rhs) {
            ...
            (Self::String { value: lhs, .. }, Self::String { value: rhs, .. }) => lhs == rhs,
            (Self::Integer { value: lhs, .. }, Self::Integer { value: rhs, .. }) => lhs == rhs,
            (Self::Float { value: lhs }, Self::Float { value: rhs }) => lhs == rhs,
