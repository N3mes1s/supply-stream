...
use protobuf::reflect::{
    EnumDescriptor, FieldDescriptor, MessageDescriptor, ReflectMapRef, ReflectRepeatedRef,
...
impl Struct {
    ...
    /// fields).
    pub fn add_field<N: Into<String>>(&mut self, name: N, value: TypeValue) -> Option<StructField> {
        let name = name.into();
    ...
    #[inline]
    pub fn field_and_index_by_name(&self, name: &str) -> Option<(&StructField, usize)> {
        self.fields
            .get_full(name)
            .map(|(index, _, field)| (field, index))
    }
        ...
        for fd in msg_descriptor.fields() {
            ...
            if Self::lowercase(&fd) {
                if let TypeValue::String { constraints, .. } = &mut value {
                    ...
                    panic!("`lowercase = true` in non-string field: {}", fd.full_name())
                }
            }
    ...
    fn is_module_root(msg_descriptor: &MessageDescriptor) -> bool {
        ...
        if let Some(options) = module_options.get(&file_descriptor.proto().options) {
    ...
    /// or the tag number.
    pub(crate) fn enum_value(enum_value_descriptor: &EnumValueDescriptor) -> EnumValue {
        enum_value
            .get(&enum_value_descriptor.proto().options)
            .and_then(|options| options.value)
            .unwrap_or_else(|| EnumValue::I64(enum_value_descriptor.value() as i64))
    }
    ...
    pub(crate) fn fields(&self) -> impl Iterator<Item = (&str, &StructField)> {
        self.fields
            .iter()
            .map(move |(name, field)| (name.as_str(), field))
    }

    /// Similar to [`Struct::enum_value`], but returns the enum value as an `i64`
    /// or `None` if it isn't an `i64`.
    pub(crate) fn enum_value_i64(enum_value_descriptor: &EnumValueDescriptor) -> Option<i64> {
    ...
    /// }];
    /// ```
    fn deprecation_notice(field_descriptor: &FieldDescriptor) -> Option<DeprecationNotice> {
        field_options
            .get(&field_descriptor.proto().options)
        ...
        match ty {
            ...
            RuntimeType::Message(msg_descriptor) => {
                let structure = if let Some(value) = value {
                    Self::from_proto_descriptor_and_value(msg_descriptor, value, enum_as_fields)
                } else {
                    Self::from_proto_descriptor_and_msg(msg_descriptor, None, enum_as_fields)
                };
        ...
        let array = match ty {
            ...
            RuntimeType::String => {
                if let Some(repeated) = repeated {
                    Array::Strings(
                        repeated
                            .into_iter()
                            .map(|value| Rc::new(BString::from(value.to_str().unwrap())))
                            .collect(),
                    )
                } else {
                    Array::Strings(vec![])
            ...
            RuntimeType::VecU8 => {
                if let Some(repeated) = repeated {
                    Array::Strings(
                        repeated
                            .into_iter()
                            .map(|value| Rc::new(BString::from(value.to_bytes().unwrap())))
                            .collect(),
                    )
                } else {
                    Array::Strings(vec![])
            ...
            RuntimeType::Message(msg_descriptor) => {
                if let Some(repeated) = repeated {
                    ...
                    Array::Structs(vec![Struct::from_proto_descriptor_and_msg(
                        msg_descriptor,
                        None,
                        enum_as_fields,
                    )])
    ...
    fn new_map(
        ...
        let map = match key_ty {
            RuntimeType::String => {
                Self::new_map_with_string_key(value_ty, map, enum_as_fields, syntax)
            }
            RuntimeType::I32 | RuntimeType::I64 | RuntimeType::U32 | RuntimeType::U64 => {
                Self::new_map_with_integer_key(value_ty, map, enum_as_fields, syntax)
            }
            ty => {
        ...
        if let Some(map) = map {
            ...
            for (key, value) in map.into_iter() {
                result.insert(
                    ...
                    Self::new_value(value_ty, Some(value), enum_as_fields, syntax),
                );
            }
            Map::IntegerKeys {
                deputy: None,
            ...
            Map::IntegerKeys {
                deputy: Some(Self::new_value(value_ty, None, enum_as_fields, syntax)),
                map: Default::default(),
            }
        }
    }
    ...
    fn from_proto_descriptor_and_value(
        ...
        if let ReflectValueRef::Message(m) = value {
            Struct::from_proto_descriptor_and_msg(msg_descriptor, Some(m.deref()), enum_as_fields)
        } else {
            unreachable!()
        }
    }
...
impl From<&Module> for Rc<Struct> {
    ...
    fn from(module: &Module) -> Self {
        ...
        let mut module_struct =
            Struct::from_proto_descriptor_and_msg(&module.root_struct_descriptor, None, true);
        ...
        let module_struct_mut = Rc::<Struct>::get_mut(&mut module_struct).unwrap();
...
mod tests {
    ...
    fn test_proto_struct() {
        ...
        let mut structure =
            Struct::from_proto_descriptor_and_msg(&TestProto2::descriptor(), None, true);
