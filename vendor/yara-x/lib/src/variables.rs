...
pub enum VariableError {
    ...
    #[error("invalid type for `{variable}`, expecting `{expected_type}`, got `{actual_type}")]
    InvalidType {
        /// Variable name.
        variable: String,
...
impl TryFrom<&serde_json::Value> for Variable {
    ...
    fn try_from(value: &serde_json::Value) -> Result<Self, Self::Error> {
        match value {
            ...
            serde_json::Value::Bool(b) => Ok(Variable(TypeValue::var_bool_from(*b))),
            serde_json::Value::Number(n) => {
                if let Some(n) = n.as_u64() {
                    let n: i64 = n.try_into().map_err(|_| VariableError::IntegerOutOfRange)?;
            ...
            serde_json::Value::String(s) => Ok(Variable(TypeValue::var_string_from(s))),
            ...
            serde_json::Value::Object(obj) => {
                ...
                for (key, value) in obj {
                    if !is_valid_identifier(key) {
                        return Err(VariableError::InvalidIdentifier(key.to_string()));
                    }
                    s.add_field(key, TypeValue::from(Variable::try_from(value)?));
                }
