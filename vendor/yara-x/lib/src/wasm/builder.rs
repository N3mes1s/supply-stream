...
use walrus::{FunctionBuilder, FunctionId, GlobalId, InstrSeqBuilder, MemoryId, Module};
...
impl WasmModuleBuilder {
    ...
    pub fn new() -> Self {
        ...
        for export in super::wasm_exports() {
            ...
            let (func_id, _) =
                module.add_import_func(export.rust_module_path, fully_qualified_name.as_str(), ty);
        ...
        let mut namespace_func = FunctionBuilder::new(&mut module.types, &[], &[]);

        let rules_func = FunctionBuilder::new(&mut module.types, &[], &Self::RULES_FUNC_RET);
    ...
    pub fn start_rule(&mut self, rule_id: RuleId, global: bool) -> InstrSeqBuilder<'_> {
    ...
    pub fn finish_rule(&mut self) {
        let rule_no_match = self.function_id(wasm::export__rule_no_match.mangled_name);

        let rule_match = self.function_id(wasm::export__rule_match.mangled_name);
    ...
    pub fn build(mut self) -> walrus::Module {
        ...
        let main_func = self.main_func.finish(Vec::new(), &mut self.module.funcs);
...
impl WasmModuleBuilder {
    ...
    fn finish_namespace_block(&mut self) {
            ...
            .is_empty()
        {
            ...
            // new block.
            self.namespace_func.func_body().instr(Block {
                seq: self.namespace_block,
            });

            self.namespace_block = self.namespace_func.dangling_instr_seq(None).id();
        }
    }
    ...
    fn finish_namespace_func(&mut self) {
        ...
        self.main_func.func_body().call(
            self.module
                .funcs
                .add_local(namespace_func.local_func(Vec::new())),
        );
    }
    ...
    fn finish_rule_func(&mut self) {
        let mut rule_func = mem::replace(
            &mut self.rules_func,
            FunctionBuilder::new(&mut self.module.types, &[], &Self::RULES_FUNC_RET),
        );
        ...
        if !rule_func.func_body().instrs().is_empty() {
            ...
            let mut namespace_block = self.namespace_func.instr_seq(self.namespace_block);

            namespace_block.call(
                self.module
                    .funcs
                    .add_local(rule_func.local_func(Vec::new())),
            );
