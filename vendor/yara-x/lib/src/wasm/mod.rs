...
use crate::types::{Array, Func, FuncSignature, Map, Struct, TypeValue, Value};
use crate::wasm::integer::RangedInteger;
use crate::wasm::runtime::{
    AsContext, AsContextMut, Caller, Config, Engine, FuncType, Linker, ValRaw, ValType,
};
...
impl WasmExport {
    ...
    {
        let mut functions: FxHashMap<&'static str, Func> = FxHashMap::default();
    ...
    pub fn get_methods(type_name: &str) -> FxHashMap<&'static str, Func> {
        WasmExport::get_functions(|export| export.method_of.is_some_and(|name| name == type_name))
    }
}
...
type TrampolineFn = Box<
    dyn Fn(Caller<'_, ScanContext>, &mut [ValRaw]) -> anyhow::Result<()> + Send + Sync + 'static,
>;
...
fn type_id_to_wasmtime(type_id: TypeId, type_name: &'static str) -> &'static [ValType] {
...
#[wasm_export(sync = "both")]
pub(crate) fn rule_match(caller: &mut Caller<'_, ScanContext>, rule_id: RuleId) {
    caller.data_mut().track_rule_match(rule_id);
}

/// Invoked from WASM to notify when a rule doesn't match.
#[wasm_export(sync = "both")]
pub(crate) fn rule_no_match(caller: &mut Caller<'_, ScanContext>, rule_id: RuleId) {
...
pub(crate) fn pat_range_match(
    ...
    let range: RangeInclusive<usize> = pattern_id_start.into()..=pattern_id_end.into();
...
#[wasm_export(sync = "none")]
pub(crate) fn pat_matches(caller: &mut Caller<'_, ScanContext>, pattern_id: PatternId) -> i64 {
    if let Some(matches) = caller.data().pattern_matches.get(pattern_id) {
...
pub(crate) fn string_len(caller: &mut Caller<'_, ScanContext>, string: RuntimeString) -> i64 {
...
pub(crate) fn array_len(_: &mut Caller<'_, ScanContext>, array: Rc<Array>) -> i64 {
    array.len() as i64
}
...
fn lookup_field(
    ...
    let lookup_indexes_ptr = unsafe { mem_ptr.offset(LOOKUP_INDEXES_START as isize) };
    ...
    let mut structure = structure
        .as_deref()
        .unwrap_or(&store_ctx.data().root_struct);
    ...
    for field_index in lookup_indexes {
        ...
        let field = structure
            ...
            .unwrap_or_else(|| {
                panic!("expecting field with index {field_index} in {structure:#?}")
            });
...
macro_rules! gen_lookup_fn {
    ($name:ident, $return_type:ty, $type:path) => {
        ...
        pub(crate) fn $name(
            ...
            if let $type { value, .. } = lookup_field(caller, structure, num_lookup_indexes) {
...
macro_rules! gen_map_lookup_fn {
    ...
    ($name:ident, RuntimeString, i64) => {
        gen_map_lookup_fn!($name, RuntimeString, i64, with_string_keys, as_integer);
    };
    ($name:ident, RuntimeString, f64) => {
        gen_map_lookup_fn!($name, RuntimeString, f64, with_string_keys, as_float);
    };
    ($name:ident, RuntimeString, bool) => {
        gen_map_lookup_fn!($name, RuntimeString, bool, with_string_keys, as_bool);
    };
...
pub(crate) fn map_lookup_by_index_string_string(
    ...
    map.with_string_keys()
        .get_index(index as usize)
        .map(|(key, value)| (Rc::new(key.as_bstr().to_owned()), value.as_string()))
        .unwrap()
}
...
pub(crate) fn map_lookup_by_index_string_struct(
    ...
    map.with_string_keys()
        .get_index(index as usize)
        .map(|(key, value)| (Rc::new(key.as_bstr().to_owned()), value.as_struct()))
        .unwrap()
}
...
#[wasm_export(sync = "none")]
pub(crate) fn str_len(caller: &mut Caller<'_, ScanContext>, s: RuntimeString) -> i64 {
    ...
    ($name:ident, $return_type:ty, $from_fn:ident, $min:expr, $max:expr) => {
        ...
        pub(crate) fn $name(
            ...
            caller
                ...
                .map(|bytes| <$return_type>::$from_fn(bytes.try_into().unwrap()) as i64)
...
macro_rules! gen_float_fn {
    ($name:ident, $return_type:ty, $from_fn:ident) => {
        #[wasm_export(public = true, sync = "none")]
        pub(crate) fn $name(caller: &mut Caller<'_, ScanContext>, offset: i64) -> Option<f64> {
            let offset = usize::try_from(offset).ok()?;
            caller
                .data()
                .scanned_data()?
                .get(offset..offset + mem::size_of::<$return_type>())
                .map(|bytes| <$return_type>::$from_fn(bytes.try_into().unwrap()) as f64)
        }
    };
}
