...
pub(crate) use super::common::{AsContext, AsContextMut, Config, Engine, OptLevel};
...
pub(crate) use super::common::{
    Extern, FuncType, Global, GlobalType, MemoryType, Mutability, Val, ValRaw, ValType,
};
...
pub(crate) struct RuntimeState {
    ...
    import_callbacks: Vec<Closure<dyn FnMut(JsValue, JsValue, JsValue, JsValue) -> JsValue>>,
}
...
impl RuntimeBackend for Backend {
    ...
    fn create_global(
        ...
        let js_global =
            WebAssembly::Global::new(&descriptor, &val_to_js(value)).map_err(js_error)?;
    ...
    fn set_global(runtime: &mut Self::RuntimeState, id: usize, value: Val) -> Result<()> {
        let inner = &runtime.globals[id];
    ...
    fn create_memory(runtime: &mut Self::RuntimeState, ty: MemoryType) -> Result<usize> {
        let descriptor = Object::new();
        Reflect::set(
            &descriptor,
        ...
        let js_memory = WebAssembly::Memory::new(&descriptor).map_err(js_error)?;
    ...
    fn memory_data<'a>(runtime: &'a Self::RuntimeState, id: usize) -> &'a [u8] {
        runtime.memories[id].cache.as_slice()
    }

    fn memory_data_mut<'a>(runtime: &'a mut Self::RuntimeState, id: usize) -> &'a mut [u8] {
        runtime.memories[id].cache.as_mut_slice()
    }

    fn memory_data_ptr(runtime: &mut Self::RuntimeState, id: usize) -> *mut u8 {
        runtime.memories[id].cache.as_mut_ptr()
    }

    fn module_from_binary(_engine: &Engine, bytes: &[u8]) -> Result<Self::ModuleInner> {
    ...
    fn instantiate<T: 'static>(
        ...
        for defined in &linker.externs {
            ...
            let value: JsValue = match defined.value {
                Extern::Global(global) => store.runtime.globals[global.id].js_global.clone().into(),
                Extern::Memory(memory) => {
                    store.runtime.memories[memory.id].js_memory.clone().into()
                }
            };

            Reflect::set(&ns, &JsValue::from_str(defined.name.as_str()), &value)
                .map_err(js_error)?;
        }
        ...
        for import in &linker.functions {
            ...
            let callback = Closure::wrap(Box::new(
                move |a0: JsValue, a1: JsValue, a2: JsValue, a3: JsValue| -> JsValue {
                    ...
                    let mut args_and_results =
                        vec![ValRaw::default(); common::callback_storage_len(&params, &results)];

                    let incoming = [a0, a1, a2, a3];

                    for (idx, ty) in params.iter().enumerate() {
                        args_and_results[idx] =
                            js_to_valraw(&incoming[idx], *ty).unwrap_or_default();
                    }
                    ...
                    match results.len() {
                        ...
                        _ => {
                            ...
                            for (idx, ty) in results.iter().enumerate() {
                                js.push(&valraw_to_js(args_and_results[idx], *ty));
                            }
                            js.into()
                        }
                    }
        ...
        let js_module = WebAssembly::Module::new(&bytes.into()).map_err(js_error)?;
    ...
    fn get_typed_func_handle<P, R>(
        ...
        let value = Reflect::get(&exports, &JsValue::from_str(name)).map_err(js_error)?;
        value
            .dyn_into::<Function>()
            .map_err(|_| anyhow!("export `{name}` is not a function"))
    }

    fn typed_func_call_i32<T>(store: &mut Store<T>, func: &Self::TypedFuncHandle) -> Result<i32> {
...
fn js_to_valraw(value: &JsValue, ty: ValType) -> Result<ValRaw> {
    Ok(match ty {
        ...
        ValType::F32 => ValRaw::f32((value.as_f64().unwrap_or(0.0) as f32).to_bits()),
...
fn valraw_to_js(value: ValRaw, ty: ValType) -> JsValue {
    match ty {
        ...
        ValType::F32 => JsValue::from_f64(f32::from_bits(value.get_f32()) as f64),
...
fn js_to_val(value: &JsValue, ty: ValType) -> Result<Val> {
    Ok(match ty {
        ...
        ValType::F32 => Val::F32((value.as_f64().unwrap_or(0.0) as f32).to_bits()),
