...
pub(crate) fn callback_storage_len(params: &[ValType], results: &[ValType]) -> usize {
...
pub(crate) trait RuntimeBackend: Clone + Default + Sized + 'static {
    ...
    fn create_global(runtime: &mut Self::RuntimeState, ty: GlobalType, value: Val)
    -> Result<usize>;
    ...
    fn module_from_binary(engine: &Engine, bytes: &[u8]) -> Result<Self::ModuleInner>;
...
pub(crate) trait AsContextMut: AsContext {
    ...
    fn as_context_mut(&mut self) -> StoreContextMut<'_, Self::Data, Self::Backend>;
}
...
impl<T: 'static, B: RuntimeBackend> Linker<T, B> {
    ...
    pub unsafe fn func_new_unchecked(
        ...
        sync_flags: u32,
        trampoline: Box<
            dyn Fn(Caller<'_, T, B>, &mut [ValRaw]) -> Result<()> + Send + Sync + 'static,
        >,
    ) -> Result<()> {
...
impl Global {
    ...
    pub fn new<T, B: RuntimeBackend>(
        ...
        Ok(Self {
            id: B::create_global(&mut store.runtime, ty, value)?,
        })
    }

    /// Returns the current value of the global.
    pub fn get<T, B: RuntimeBackend>(&self, store: StoreContextMut<'_, T, B>) -> Val {
...
impl Memory {
    ...
    pub fn new<T, B: RuntimeBackend>(
        ...
        Ok(Self {
            id: B::create_memory(&mut store.runtime, ty)?,
        })
    }

    /// Returns the current contents of the memory.
    pub fn data<'a, T, B: RuntimeBackend>(&self, store: StoreContext<'a, T, B>) -> &'a [u8] {
    ...
    pub fn data_ptr<T, B: RuntimeBackend>(&self, store: StoreContextMut<'_, T, B>) -> *mut u8 {
...
impl<B: RuntimeBackend> TypedFunc<(), i32, B> {
    ...
    pub fn call<T>(&self, store: StoreContextMut<'_, T, B>, _params: ()) -> Result<i32> {
