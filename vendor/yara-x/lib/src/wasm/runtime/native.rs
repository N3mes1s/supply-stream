...
pub(crate) use wasmtime::{
    AsContext, AsContextMut, Caller, Config, Engine, Extern, FuncType, Global, GlobalType,
    Instance, Memory, MemoryType, Module, Mutability, OptLevel, Store, TypedFunc, Val, ValRaw,
    ValType,
};
...
type Trampoline<T> = dyn Fn(Caller<'_, T>, &mut [ValRaw]) -> Result<()> + Send + Sync + 'static;
...
impl<T: 'static> Linker<T> {
    ...
    pub unsafe fn func_new_unchecked(
        ...
        unsafe {
            self.0
                .func_new_unchecked(module, name, ty, trampoline)
                .map(|_| ())
        }
    }
