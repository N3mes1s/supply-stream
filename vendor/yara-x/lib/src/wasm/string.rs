...
use crate::scanner::{RuntimeObject, RuntimeObjectHandle, ScanContext, ScanState};
        ...
        match self {
            ...
            Self::ScannedDataSlice { offset, length } => {
                if length >= u16::MAX as usize {
                    panic!("runtime-string slices can't be larger than {}", u16::MAX)
                }
...
impl RuntimeString {
    ...
    pub(crate) fn as_bstr<'a>(&'a self, ctx: &'a ScanContext) -> &'a BStr {
        match self {
            Self::Literal(id) => ctx.compiled_rules.lit_pool().get(*id).unwrap(),
            Self::ScannedDataSlice { offset, length } => {
    ...
    #[inline]
    pub(crate) fn to_str<'a>(&'a self, ctx: &'a ScanContext) -> Result<&'a str, Utf8Error> {
    ...
    pub(crate) fn from_wasm(ctx: &mut ScanContext, s: RuntimeStringWasm) -> Self {
        match s & 0x3 {
    ...
    #[inline]
    pub(crate) fn contains(&self, other: &Self, ctx: &ScanContext, case_insensitive: bool) -> bool {
        if case_insensitive {
    ...
    #[inline]
    pub(crate) fn equals(&self, other: &Self, ctx: &ScanContext, case_insensitive: bool) -> bool {
        if case_insensitive {
            let this = self.as_bstr(ctx).to_lowercase();
            let other = other.as_bstr(ctx).to_lowercase();
            this.eq(&other)
