...
pub(crate) fn impl_error_struct_macro(input: DeriveInput) -> Result<TokenStream> {
    let fields = match &input.data {
        Data::Struct(s) => &s.fields,
        Data::Enum(_) | Data::Union(_) => {
            return Err(Error::new(
                input.ident.span(),
                "macro ErrorStruct can be used with only with struct types".to_string(),
            ));
        }
    };
            ...
            attr.parse_nested_meta(|meta| {
                match meta.path.get_ident() {
                    ...
                    _ => {
                        return Err(Error::new(
                            meta.path.span(),
                            "unknown argument, expecting `code = \"...\", title = \"...\"`"
                                .to_string(),
                        ));
                    }
                };
    ...
    let fields: Vec<&Field> = fields
        .iter()
        .filter(|field| field.ident.as_ref().is_some_and(|ident| ident != "report"))
        .collect();
...
pub(crate) fn impl_error_enum_macro(input: DeriveInput) -> Result<TokenStream> {
    let variants = match &input.data {
        Data::Enum(s) => &s.variants,
        Data::Struct(_) | Data::Union(_) => {
            return Err(Error::new(
                input.ident.span(),
                "macro ErrorEnum can be used with only with enum types".to_string(),
            ));
        }
    };
