...
use syn::visit::Visit;
use syn::{
    AngleBracketedGenericArguments, Error, Expr, ExprLit, GenericArgument, Ident, ItemFn, Lit,
    PatType, PathArguments, Result, ReturnType, Type, TypePath,
};
...
impl<'ast> FuncSignatureParser<'ast> {
    ...
    #[inline(always)]
    fn type_args(type_path: &TypePath) -> Result<impl Iterator<Item = &GenericArgument>> {
    ...
    fn type_args_as_integers(type_path: &TypePath, error_msg: &str) -> Result<Vec<i64>> {
    ...
    fn type_path_to_mangled_named(type_path: &TypePath) -> Result<Cow<'static, str>> {
        match Self::type_ident(type_path).to_string().as_str() {
            ...
            "RangedInteger" => {
                let error_msg =
                    "RangedInteger must have MIN and MAX arguments (i.e: RangedInteger<0,256>)";
            ...
            type_ident => Err(Error::new_spanned(
                type_path,
                format!("type `{type_ident}` is not supported as argument or return type"),
            )),
        }
    }
            ...
            Type::Path(type_path) => {
                if Self::type_ident(type_path) == "Option" {
                    if let PathArguments::AngleBracketed(angle_bracketed) =
                        &type_path.path.segments.last().unwrap().arguments
                    {
                        if let GenericArgument::Type(ty) = angle_bracketed.args.first().unwrap() {
...
impl<'ast> Visit<'ast> for FuncSignatureParser<'ast> {
    fn visit_pat_type(&mut self, pat_type: &'ast PatType) {
        ...
        self.args
            .as_mut()
            .unwrap()
            .push_back((name, pat_type.ty.as_ref()));
    }
}
...
fn sync_flags_literal(sync: Option<&str>, default: &str) -> Result<TokenStream> {
    ...
    let bits = match sync {
        ...
        _ => {
            return Err(Error::new(
                ...
                format!("invalid sync mode `{sync}`, expected one of: none, before, after, both"),
            ));
        }
    };
    Ok(quote! { #bits })
