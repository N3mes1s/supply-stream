...
pub(crate) fn rule_ascii_tree(rule: &Rule) -> Tree {
    ...
    if let Some(meta) = &rule.meta {
        rule_children.push(Node(
            "meta".to_owned(),
            meta.iter()
                .map(|m| Leaf(vec![format!("{} = {}", m.identifier.name, m.value)]))
                .collect(),
        ))
    }
...
fn build_tree_for_expr(expr: &Expr, children: Vec<Tree>) -> Tree {
    match expr {
        ...
        Expr::Regexp(regexp) => match (regexp.case_insensitive, regexp.dot_matches_new_line) {
                ...
                match anchor {
                    ...
                    MatchAnchor::In(_) => {
                        ...
                        Node(
                            ...
                            vec![
                                Node("<start>".to_string(), vec![children_iter.next().unwrap()]),
                                Node("<end>".to_string(), vec![children_iter.next().unwrap()]),
                            ],
                        )
                    }
                }
        ...
        Expr::Lookup(_) => {
            ...
            Node(
                ...
                vec![
                    Node("<expr>".to_string(), vec![children_iter.next().unwrap()]),
                    Node("<index>".to_string(), vec![children_iter.next().unwrap()]),
                ],
            )
        }
        ...
        Expr::FuncCall(expr) => {
            ...
            for label in labelled_args {
                new_children.push(Node(label, vec![children_iter.next().unwrap()]));
            }
        ...
        Expr::Of(of) => {
            ...
            let quantifier = quantifier_ascii_tree(&of.quantifier, children_iter.by_ref());

            let mut new_children = vec![Node("<quantifier>".to_string(), vec![quantifier])];
            ...
            let node_title = if let Some(anchor) = &of.anchor {
                match anchor {
                    ...
                    MatchAnchor::In(_) => {
                        ...
                        "<quantifier> of <items> in (<start>..<end>)".to_string()
                    }
                }
            } else {
        ...
        Expr::ForIn(f) => {
            ...
            let mut new_children = vec![
                Node(
                    "<quantifier>".to_string(),
                    vec![quantifier_ascii_tree(&f.quantifier, children_iter.by_ref())],
                ),
                Node(
                    "<vars>".to_string(),
                    vec![Leaf(
                        f.variables.iter().map(|v| v.name.to_string()).collect(),
                    )],
                ),
            ];
            ...
            let node_title = match &f.iterable {
                ...
                Iterable::ExprTuple(args) => {
                    let labelled_args: Vec<String> =
                        (0..args.len()).map(|i| format!("<expr{i}>")).collect();
                ...
                Iterable::Expr(_) => {
                    ...
                    "for <quantifier> <vars> in <expr> : ( <condition> )".to_string()
                }
            };
                ...
                vec![
                    Node(
                        ...
                        w.declarations
                            ...
                            .flat_map(|d| {
                                vec![
                                    Leaf(vec![format!("{}", d.identifier.name)]),
...
pub(crate) fn quantifier_ascii_tree(
    ...
    match quantifier {
        ...
        Quantifier::Percentage(_) => Node("percentage".to_string(), vec![children.next().unwrap()]),
    }
}
...
pub(crate) fn pattern_set_ascii_tree(pattern_set: &PatternSet) -> Tree {
    match pattern_set {
        ...
        PatternSet::Set(set) => Leaf(set.iter().map(|s| s.identifier.to_string()).collect()),
    }
}
...
pub(crate) fn pattern_ascii_tree(pattern: &Pattern) -> Tree {
    match pattern {
        ...
        Pattern::Regexp(r) => Leaf(vec![format!(
            ...
            if r.regexp.dot_matches_new_line {
                "s"
            } else {
                ""
            },
...
pub(crate) fn hex_tokens_ascii_tree(tokens: &HexSubPattern) -> Tree {
    let nodes = tokens
        .iter()
        .map(|t| match t {
            HexToken::Byte(b) => Leaf(vec![format!("{:#04X} mask: {:#04X}", b.value, b.mask)]),
            HexToken::NotByte(b) => Leaf(vec![format!("~ {:#04X} mask: {:#04X}", b.value, b.mask)]),
