    ...
    I: Iterator<Item = Event>,
{
    ...
    fn recover(&mut self) {
        loop {
            match self.peek() {
                ...
                _ => {
                    if let Some(Event::Error { message, span }) = self.events.next() {
                        self.errors.push(Error::SyntaxError { message, span });
                    }
                }
            }
    ...
    fn get_source_str(&mut self, span: &Span) -> Result<&'src str, BuilderError> {
    ...
    fn expect(&mut self, expected_kind: SyntaxKind) -> Result<Span, BuilderError> {
        match self.next()? {
    ...
    I: Iterator<Item = Event>,
{
    fn include_stmt(&mut self) -> Result<Include<'src>, BuilderError> {
        ...
        Ok(Include {
            file_name,
            span: span.combine(&file_name_span),
        })
    }
    ...
    fn import_stmt(&mut self) -> Result<Import<'src>, BuilderError> {
        ...
        Ok(Import {
            module_name,
            span: span.combine(&module_name_span),
        })
    }
    ...
    fn rule_decl(&mut self) -> Result<Rule<'src>, BuilderError> {
        ...
        let patterns = if let Event::Begin {
            kind: PATTERNS_BLK, ..
        } = self.peek()
        {
    ...
    fn meta_def(&mut self) -> Result<Meta<'src>, BuilderError> {
        ...
        let multiplier: i64 = if matches!(self.peek(), &Event::Token { kind: MINUS, .. }) {
            self.expect(MINUS)?;
            -1
        } else {
            1
        ...
        let value = match self.peek() {
            ...
            Event::Token {
                ...
                match self.string_lit(true)? {
                    ...
                    // string.
                    (Cow::Borrowed(s), _lit, span) => {
                        MetaValue::String((unsafe { s.to_str_unchecked() }, span))
                    }
            ...
            Event::Token { kind: TRUE_KW, .. } => MetaValue::Bool((true, self.expect(TRUE_KW)?)),
            Event::Token { kind: FALSE_KW, .. } => MetaValue::Bool((false, self.expect(FALSE_KW)?)),
            event => panic!("unexpected {event:?}"),
        };
    ...
    fn pattern_mods_opt(&mut self) -> Result<PatternModifiers<'src>, BuilderError> {
        if let Event::Begin {
            kind: PATTERN_MODS, ..
        } = self.peek()
        {
    ...
    fn pattern_mods(&mut self) -> Result<PatternModifiers<'src>, BuilderError> {
                ...
                token @ Event::Token {
                    ...
                    match token {
                        Event::Token {
                            ...
                            modifiers.push(PatternModifier::Base64 { span, alphabet });
                        }
                        Event::Token {
                            kind: BASE64WIDE_KW,
                            span,
                        } => {
                            modifiers.push(PatternModifier::Base64Wide { span, alphabet });
                        }
                        event => panic!("unexpected {event:?}"),
                    };
                }
    ...
    fn hex_sub_pattern(&mut self) -> Result<HexSubPattern, BuilderError> {
        ...
        loop {
            sub_patterns.push(match self.peek() {
                ...
                Event::Begin { kind: HEX_JUMP, .. } => HexToken::Jump(self.hex_jump()?),
                _ => break,
            });
        }
    ...
    fn boolean_term(&mut self) -> Result<Expr<'src>, BuilderError> {
        ...
        let expr = match self.peek() {
            ...
            Event::Token {
                ...
                let term = self.boolean_term()?;
                let span = span.combine(&term.span());
                Expr::Defined(Box::new(UnaryExpr {
                    operand: term,
                    span,
                }))
            }
            ...
            Event::Begin { kind: EXPR, .. } => self.pratt_parser(Self::expr, 0)?,
            event => panic!("unexpected {event:?}"),
        };
        ...
        match self.peek() {
            Event::Token { kind: OF_KW, .. } => {
                ...
                pattern_set = match self.peek() {
                    Event::Token { kind: THEM_KW, .. } => Some(PatternSet::Them {
    ...
    fn of_expr(&mut self) -> Result<Expr<'src>, BuilderError> {
        ...
        let items = match self.peek() {
            Event::Token { kind: THEM_KW, .. } => OfItems::PatternSet(PatternSet::Them {
                span: self.expect(THEM_KW)?,
            }),
            Event::Begin {
                kind: PATTERN_IDENT_TUPLE,
                ..
            } => OfItems::PatternSet(PatternSet::Set(self.pattern_ident_tuple()?)),
            Event::Begin {
    ...
    fn with_expr(&mut self) -> Result<Expr<'src>, BuilderError> {
        ...
        let declaration = |i: &mut Self| -> Result<WithDeclaration<'src>, BuilderError> {
            i.begin(WITH_DECL)?;
    ...
    fn quantifier(&mut self) -> Result<Quantifier<'src>, BuilderError> {
        ...
        let quantifier = match self.peek() {
            Event::Token { kind: ALL_KW, .. } => Quantifier::All {
                span: self.expect(ALL_KW)?,
            },
            Event::Token { kind: NONE_KW, .. } => Quantifier::None {
                span: self.expect(NONE_KW)?,
            },
            Event::Token { kind: ANY_KW, .. } => Quantifier::Any {
                span: self.expect(ANY_KW)?,
            },
    ...
    fn primary_expr(&mut self) -> Result<Expr<'src>, BuilderError> {
        ...
        let mut expr = match self.peek() {
            ...
            Event::Token { kind: IDENT, .. } => Expr::Ident(Box::new(self.identifier()?)),
    ...
    fn func_call(&mut self, object: Option<Expr<'src>>) -> Result<Expr<'src>, BuilderError> {
    ...
    fn term(&mut self) -> Result<Expr<'src>, BuilderError> {
        ...
        let expr = match self.peek() {
            ...
            Event::Token { kind: REGEXP, .. } => Expr::Regexp(Box::new(self.regexp()?)),
            Event::Token {
                kind: FILESIZE_KW, ..
            } => Expr::Filesize {
                span: self.expect(FILESIZE_KW)?,
            },
            Event::Token {
                kind: ENTRYPOINT_KW,
                ..
            } => Expr::Entrypoint {
                span: self.expect(ENTRYPOINT_KW)?,
            },
            Event::Token {
                kind: PATTERN_COUNT,
                ..
                ...
                let (span_with_range, range) = if let Event::Token { kind: IN_KW, .. } = self.peek()
                {
                    self.expect(IN_KW)?;
                    let range = self.range()?;
            ...
            Event::Token { kind: MINUS, .. } => {
                ...
                match operand {
                    ...
                    Expr::LiteralInteger(mut integer)
                        if integer.value.is_positive() && !literal.contains('(') =>
                    {
            ...
            Event::Begin {
                ...
                let mut coalesced: Vec<Expr> = exprs
                    ...
                    .coalesce(|x, y| match (x, y) {
                        ...
                        (field_access @ Expr::FieldAccess(_), Expr::FuncCall(mut func_call)) => {
                        ...
                        (ident @ Expr::Ident(_), Expr::FuncCall(mut func_call)) => {
                        ...
                        (ident @ Expr::Ident(_), Expr::Lookup(mut lookup)) => {
                            ...
                            lookup.primary = Expr::FieldAccess(Box::new(NAryExpr::from(vec![
                                ident,
                                lookup.primary,
                            ])));
                        ...
                        (Expr::FieldAccess(mut fa), Expr::Lookup(mut lookup)) => {
    ...
    fn pattern_ident_tuple(&mut self) -> Result<Vec<PatternSetItem<'src>>, BuilderError> {
        self.begin(PATTERN_IDENT_TUPLE)?;
        self.expect(L_PAREN)?;

        let item = |s: &mut Self| -> Result<PatternSetItem<'src>, BuilderError> {
            let ident = s.pattern_ident()?;
            let mut span = ident.span();
            let wildcard = if matches!(s.peek(), Event::Token { kind: ASTERISK, .. }) {
        ...
        T: Num + Bounded + CheckedMul + FromPrimitive + std::fmt::Display,
    {
        ...
        let value = if literal_no_underscores.as_str().starts_with("0x") {
            T::from_str_radix(literal_no_underscores.strip_prefix("0x").unwrap(), 16)
        } else if literal.starts_with("0o") {
            T::from_str_radix(literal_no_underscores.strip_prefix("0o").unwrap(), 8)
        } else {
    ...
    fn float_lit(&mut self) -> Result<(f64, &'src str, Span), BuilderError> {
        ...
        let literal = self.get_source_str(&span)?;
        let value = literal.replace('_', "").parse::<f64>().map_err(|err| {
    ...
    fn regexp(&mut self) -> Result<Regexp<'src>, BuilderError> {
        ...
        for (i, modifier) in re[closing_slash + 1..].char_indices() {
            match modifier {
                ...
                c => {
                    let span =
                        span.subspan(closing_slash + 1 + i, closing_slash + 1 + i + c.len_utf8());
    ...
    fn utf8_string_lit(&mut self) -> Result<(&'src str, Span), BuilderError> {
        ...
        match self.string_lit(false)? {
            (Cow::Borrowed(a), _, span) => unsafe { Ok((a.to_str_unchecked(), span)) },
            _ => unreachable!(),
        }
    }
                ...
                '\\' => {
                    ...
                    match escaped_char.1 {
                        ...
                        'x' => match (chars.next(), chars.next()) {
                            (Some((start, first_char)), Some((end, second_char)))
                                if first_char.is_ascii_hexdigit()
                                    && second_char.is_ascii_hexdigit() =>
                            {
                                let hex_value =
                                    u8::from_str_radix(&remaining[start..=end], 16).unwrap();
                            ...
                            _ => {
                                ...
                                self.errors.push(Error::InvalidEscapeSequence {
                                    message: r"expecting two hex digits after `\x`".to_string(),
                                    span: string_span
                                        .offset(first_backslash as isize)
                                        .subspan(backslash, escaped_char_pos + 1),
                                });
                        ...
                        _ => {
                            ...
                            let escaped_char_end_pos = escaped_char_pos + escaped_char.len_utf8();

                            self.errors.push(Error::InvalidEscapeSequence {
                                message: format!(
                                    "invalid escape sequence `{}`",
                                    &remaining[backslash..escaped_char_end_pos]
                                ),
                                span: string_span
                                    .offset(first_backslash as isize)
