...
impl<'src> DFSIter<'src> {
    ...
    /// ```
    pub fn contexts(&self) -> impl DoubleEndedIterator<Item = &DFSContext<'src>> {
        itertools::chain(
...
fn dfs_enter<'a>(expr: &'a Expr, stack: &mut Vec<DFSEvent<(&'a Expr<'a>, DFSContext<'a>)>>) {
    match expr {
        Expr::True { .. }
        | Expr::False { .. }
        | Expr::Filesize { .. }
        ...
        Expr::PatternCount(p) => {
            if let Some(r) = &p.range {
                stack.push(DFSEvent::Enter((&r.upper_bound, DFSContext::Range(expr))));
                stack.push(DFSEvent::Enter((&r.lower_bound, DFSContext::Range(expr))));
            }
        }
        ...
        Expr::PatternMatch(m) => {
            if let Some(anchor) = &m.anchor {
                match anchor {
                    MatchAnchor::At(at) => {
                        stack.push(DFSEvent::Enter((&at.expr, DFSContext::Anchor(expr))));
                    }
        ...
        Expr::Lookup(lookup) => {
            stack.push(DFSEvent::Enter((&lookup.index, DFSContext::Index(expr))));
            stack.push(DFSEvent::Enter((
                &lookup.primary,
        ...
        Expr::FieldAccess(e) => {
            for operand in e.operands.iter().rev() {
                stack.push(DFSEvent::Enter((operand, DFSContext::Operand(expr))));
            }
        }
        ...
        Expr::Defined(e) | Expr::Not(e) | Expr::Minus(e) | Expr::BitwiseNot(e) => {
            stack.push(DFSEvent::Enter((&e.operand, DFSContext::Operand(expr))));
        }
        ...
        Expr::Add(e) | Expr::Sub(e) | Expr::Mul(e) | Expr::Div(e) | Expr::Mod(e) => {
            for operand in e.operands.iter().rev() {
                stack.push(DFSEvent::Enter((operand, DFSContext::Operand(expr))));
            }
        }
        ...
        Expr::Of(of) => {
            ...
            if let OfItems::BoolExprTuple(tuple) = &of.items {
                for item in tuple.iter().rev() {
                    stack.push(DFSEvent::Enter((item, DFSContext::Items(expr))));
                }
            }
            match &of.quantifier {
                Quantifier::Percentage(quantifier) | Quantifier::Expr(quantifier) => {
                    stack.push(DFSEvent::Enter((quantifier, DFSContext::Quantifier(expr))));
                }
                _ => {}
            }
        }
        ...
        Expr::ForOf(for_of) => {
            stack.push(DFSEvent::Enter((&for_of.body, DFSContext::Body(expr))));
        ...
        Expr::ForIn(for_in) => {
            stack.push(DFSEvent::Enter((&for_in.body, DFSContext::Body(expr))));
            match &for_in.iterable {
                ...
                Iterable::Expr(iterable_expr) => {
                    stack.push(DFSEvent::Enter((iterable_expr, DFSContext::Items(expr))));
                }
            }
