...
use crate::cst::SyntaxKind::{
    ASCII_KW, BASE64_KW, BASE64WIDE_KW, FULLWORD_KW, NOCASE_KW, WIDE_KW, XOR_KW,
};
...
impl<'src> AST<'src> {
    ...
    /// specific code.
    #[doc(hidden)]
    pub fn new<I: Iterator<Item = Event>>(src: &'src [u8], events: I) -> AST<'src> {
...
impl<'src> PatternModifiers<'src> {
    ...
    pub fn iter(&self) -> PatternModifiersIter<'_> {
        PatternModifiersIter {
            iter: self.modifiers.iter(),
        }
    }
...
pub enum PatternModifier<'src> {
    ...
    Base64 {
        span: Span,
        alphabet: Option<LiteralString<'src>>,
    },
    Base64Wide {
        span: Span,
        alphabet: Option<LiteralString<'src>>,
    ...
    fn span(&self) -> Span {
        match self {
            ...
            PatternSet::Set(items) => {
                let span = items.first().map(|item| item.span()).unwrap_or_default();

                if items.len() == 1 {
                    return span;
                }

                span.combine(&items.last().map(|item| item.span()).unwrap_or_default())
            }
        }
    }
}
...
impl WithSpan for &Vec<Expr<'_>> {
    fn span(&self) -> Span {
        let span = self
            .first()
            .expect("calling span() on an empty Vec<Expr>")
            .span();
...
impl WithSpan for Expr<'_> {
    fn span(&self) -> Span {
        match self {
            ...
            Expr::Defined(expr) | Expr::Not(expr) | Expr::Minus(expr) | Expr::BitwiseNot(expr) => {
                expr.span()
            }
