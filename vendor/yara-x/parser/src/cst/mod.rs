        ...
        for event in self.iter.by_ref() {
            match event {
                rowan::WalkEvent::Enter(e) => {
                    return match e {
                        rowan::SyntaxElement::Node(node) => Some(Event::Begin {
                            kind: node.kind(),
                            span: Span::from(node.text_range()),
                        }),
                        rowan::SyntaxElement::Token(token) => Some(Event::Token {
                            kind: token.kind(),
...
impl CST {
    ...
    pub fn iter(&self) -> impl Iterator<Item = Event> + '_ {
        CSTIter {
            iter: rowan::SyntaxNode::new_root(self.root.clone()).preorder_with_tokens(),
...
impl<M: Clone> Token<M> {
    ...
    pub fn end_pos<E: Encoding>(&self) -> Position<E> {
        ...
        match token.rfind('\n') {
            ...
            None => Position::from((start.line, start.column + self.len::<E>())),
        }
    }
...
impl Token<Mutable> {
    ...
    pub fn replace(&mut self, text: &str) -> Node<Mutable> {
        Node::new(rowan::SyntaxNode::new_root_mut(
            self.inner
                .replace_with(rowan::GreenToken::new(self.kind().into(), text)),
        ))
    }
}
...
impl<M> From<rowan::SyntaxElement<YARA>> for NodeOrToken<M> {
    fn from(value: rowan::SyntaxElement<YARA>) -> Self {
        match value {
            ...
            rowan::SyntaxElement::Token(token) => Self::Token(Token::new(token)),
        }
    }
}
...
impl<M: Clone> Node<M> {
    ...
    /// ```
    pub fn siblings(&self, direction: Direction) -> impl Iterator<Item = Node<M>> {
    ...
    pub fn token_at_position<E: Encoding, P: Into<Position<E>>>(
        ...
        while let Some(token) = next_token {
            ...
            if position.line == line && position.column >= col && position.column < col + token_len
            {
                return Some(token);
            }
            match token.kind() {
