...
impl<'src> Parser<'src> {
    ...
    /// a stream of events.
    #[inline]
    #[deprecated(since = "1.3.0", note = "use `CSTStream::from(parser)` instead")]
    pub fn into_cst_stream(self) -> CSTStream<'src, impl Iterator<Item = Event> + use<'src>> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.state {
            ...
            _ => {
                ...
                if !matches!(self.state, State::OutOfFuel) && self.tokens.has_more() {
                    let _ = self.trivia();
...
impl<'src> ParserImpl<'src> {
    ...
    fn recover(&mut self, recovery_set: &'static TokenSet) -> &mut Self {
        ...
        if self.pending_errors.is_empty() {
            ...
            if !token_in_recovery_set {
                expected.extend(recovery_set.token_ids().map(|token| token.description()));
            }
        }
    ...
    fn expect_d(
        ...
        let (token_span, token_id, token_match) = match self.peek_non_trivia() {
            ...
            Some(token) => (token.span(), token.id(), expected_tokens.contains(token)),
        };
        ...
        match (self.not_depth, token_match) {
            ...
            (0, None) => {
                ...
                if let Some(description) = description {
                    ...
                    expected.extend(expected_tokens.token_ids().map(|token| token.description()));
                }
            }
            _ => {}
        }
    ...
    ///
    fn if_next<P>(&mut self, expected_tokens: &'static TokenSet, parser: P) -> &mut Self
    where
        P: Fn(&mut Self) -> &mut Self,
    {
        if matches!(self.state, State::Failure | State::OutOfFuel) {
    ...
    fn cond<P>(&mut self, expected_tokens: &'static TokenSet, parser: P) -> &mut Self
    where
        P: Fn(&mut Self) -> &mut Self,
    {
    ...
    fn handle_errors(&mut self) {
        ...
        let (span, expected) = match (expected_token, unexpected_token) {
            ...
            (None, Some(u)) => (u, None),
            (Some(((e, token_id), expected)), _) => (e, Some((token_id, expected))),
            (None, None) => return,
        };
        ...
        let error_msg = match from_utf8(&self.tokens.source()[span.range()]) {
            Ok(actual_token) => {
                if let Some((actual_token_id, expected)) = expected {
                    ...
                    if actual_token_id == TokenId::UNKNOWN && actual_token.starts_with("/*") {
                        "unclosed comment".to_string()
                    }
                    // When the token actually found in the source code is
                    // unknown, but starts with ", it's an unclosed literal
                    // string.
                    else if actual_token_id == TokenId::UNKNOWN && actual_token.starts_with('"') {
                    ...
                    // unknown, but starts with /, it's an unclosed regexp.
                        ...
                        let (last, all_except_last) = expected.as_slice().split_last().unwrap();
...
impl ParserImpl<'_> {
    ...
    fn top_level_item(&mut self) -> &mut Self {
        ...
        match token {
            ...
            Token::GLOBAL_KW(_) | Token::PRIVATE_KW(_) | Token::RULE_KW(_) => self.rule_decl(),
            token => {
                let span = token.span();
                self.output
                    .push_error("expecting import statement or rule definition", span);
    ...
    fn meta_def(&mut self) -> &mut Self {
        self.begin(META_DEF)
            ...
            .expect(t!(EQUAL))
            .begin_alt()
            .alt(|p| p.opt_expect(t!(MINUS)).expect(t!(INTEGER_LIT | FLOAT_LIT)))
    ...
    fn pattern_mod(&mut self) -> &mut Self {
        ...
        self.begin(PATTERN_MOD)
            ...
            .alt(|p| {
                p.expect_d(
                    t!(ASCII_KW | WIDE_KW | NOCASE_KW | PRIVATE_KW | FULLWORD_KW),
                    DESC,
                )
            })
            .alt(|p| {
    ...
    fn term(&mut self) -> &mut Self {
        ...
        self.cached(TERM, |p| {
            p.begin(TERM)
                ...
                .alt(|p| {
                    p.primary_expr()
                        .zero_or_more(|p| p.expect_d(t!(DOT), DESC).then(Self::primary_expr))
                })
                .end_alt()
                .end()
        })
    ...
    fn of_expr(&mut self) -> &mut Self {
        self.begin(OF_EXPR)
            ...
            .alt(|p| p.boolean_expr_tuple().not(|p| p.expect(t!(AT_KW | IN_KW))))
            .end_alt()
            .end()
    }
    ...
    fn quantifier(&mut self) -> &mut Self {
        self.begin(QUANTIFIER)
            .begin_alt()
            .alt(|p| p.expect_d(t!(ALL_KW | NONE_KW | ANY_KW), Some("expression")))
