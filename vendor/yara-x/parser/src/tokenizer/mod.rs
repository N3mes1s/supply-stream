...
impl<'src> Tokenizer<'src> {
            ...
            match &mut self.mode {
                ...
                Mode::HexPattern(lexer) => match lexer.next()? {
                    ...
                    Err(()) => {
                        ...
                        self.mode =
                            Mode::Normal(Logos::lexer(&self.source[self.lexer_starting_pos..]));
                    }
                },
    ...
    pub fn enter_hex_pattern_mode(&mut self) {
        ...
        self.mode = Mode::HexPattern(Logos::lexer(&self.source[self.lexer_starting_pos..]));
    }
    ...
    pub fn enter_hex_jump_mode(&mut self) {
        ...
        self.mode = Mode::HexJump(Logos::lexer(&self.source[self.lexer_starting_pos..]));
    }
}
    ...
    fn unexpected_token(&mut self) -> Token {
        ...
        if chunk.valid().is_empty() {
            return Token::INVALID_UTF8(
                Span(start as u32..(start + 1) as u32).offset(self.lexer_starting_pos as isize),
            );
        }
        ...
        Token::UNKNOWN(Span::from(lexer.span()).offset(self.lexer_starting_pos as isize))
    }
}
...
fn convert_normal_token(token: NormalToken, span: Span) -> Token {
    match token {
        ...
        NormalToken::StringLit | NormalToken::MultiLineStringLit => Token::STRING_LIT(span),

        NormalToken::Regexp => Token::REGEXP(span),

        NormalToken::BlockComment | NormalToken::Comment => Token::COMMENT(span),

        NormalToken::Whitespace => Token::WHITESPACE(span),

        NormalToken::LF | NormalToken::CR | NormalToken::CRLF => Token::NEWLINE(span),
...
fn convert_hex_pattern_token(token: HexPatternToken, span: Span) -> Token {
    match token {
        ...
        HexPatternToken::LF | HexPatternToken::CR | HexPatternToken::CRLF => Token::NEWLINE(span),
        HexPatternToken::BlockComment | HexPatternToken::Comment => Token::COMMENT(span),
    }
}
...
fn convert_hex_jump_token(token: HexJumpToken, span: Span) -> Token {
    match token {
        ...
        HexJumpToken::LF | HexJumpToken::CR | HexJumpToken::CRLF => Token::NEWLINE(span),
