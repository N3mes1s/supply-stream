...
impl<W: Write> Serializer<W> {
    ...
    fn write_value(
        ...
        match value {
            ReflectValueRef::U32(v) => self.print_integer_value(*v, get_field_format(field))?,
            ReflectValueRef::U64(v) => {
                self.print_integer_value(*v as i64, get_field_format(field))?
            }
            ReflectValueRef::I32(v) => self.print_integer_value(*v, get_field_format(field))?,
            ReflectValueRef::I64(v) => self.print_integer_value(*v, get_field_format(field))?,
    ...
    fn write_name_value_separator(&mut self, value: &ReflectValueRef) -> Result<(), Error> {
...
impl Ord for KV<'_> {
    fn cmp(&self, other: &Self) -> Ordering {
        match self.key {
            ReflectValueRef::U32(v) => v.cmp(&other.key.to_u32().unwrap_or_default()),
            ReflectValueRef::U64(v) => v.cmp(&other.key.to_u64().unwrap_or_default()),
            ReflectValueRef::I32(v) => v.cmp(&other.key.to_i32().unwrap_or_default()),
            ReflectValueRef::I64(v) => v.cmp(&other.key.to_i64().unwrap_or_default()),
            ReflectValueRef::Bool(v) => v.cmp(&other.key.to_bool().unwrap_or_default()),
            ReflectValueRef::String(v) => v.cmp(other.key.to_str().unwrap_or_default()),
            _ => {
